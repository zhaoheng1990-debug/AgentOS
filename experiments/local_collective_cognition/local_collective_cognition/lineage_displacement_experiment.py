"""Lineage-bound displacement experiment v0.47."""
...
from .lineage_displacement_holdout import (
    REPLICATION_IDS,
...
def build_lineage_displacement_preregistration(
    *, corpus, prior_analysis, prior_closure, prior_posthoc
    ...
    if (
        ...
        or prior_posthoc.get("positive_oracle_cell_count") != 1
        or prior_posthoc.get(
            "classification_distribution", {}
        ).get("HARMFUL_ACCEPTED") != 1
    ):
        raise ValueError("lineage_displacement_prior_invalid")
    ...
    commitment = {
        ...
        "frozen_hypothesis": (
            "Immutable opportunity type and evidence-warrant lineage will "
            "prevent an independent delta role from strengthening a route "
            "without new evidence. An additional source-blind pairwise "
            "displacement receipt must compare the delta with the exact "
            "base candidate it would replace. Together these controls "
            "should prevent the correlated harmful acceptance observed in "
            "v0.46 without using private outcomes."
        ),
        ...
        "lineage_displacement_adds_provider_calls": True,
...
def run_lineage_displacement_experiment(
    *, corpus, preregistration, adapter, checkpoint_callback=None
    ...
    for key, qualification in sorted(
        ...
        lineage_displacement_receipts[key] = (
            derive_admission_opportunity_receipt(
                key=key,
    ...
    for key, qualification in sorted(
        ...
        routing = lineage_displacement_receipts[key]
        ...
        delta_view = build_lineage_bound_delta_view(
            raw_receipt=provisional,
            item=item,
            qualification_receipt=qualification,
        ...
        if envelope.status != "COMPLETED":
            ...
            contract_failures = validate_lineage_bound_delta(
                delta=delta,
                item=delta_view,
                refs=refs,
            )
    ...
    commitment = {
        ...
        "lineage_displacement_receipts": lineage_displacement_receipts,
...
def analyze_lineage_displacement_experiment(*, corpus, preregistration, run):
    validate_lineage_displacement_holdout(corpus)
    _validate_preregistration(preregistration, corpus)
    ...
    conditions = {
        ...
        "compact_delta_boundary_preserved": (
            ...
            and run["lineage_displacement_adds_provider_calls"] is True
    ...
    if passed:
        decision = "PASS_ADMISSION_LINEAGE_DISPLACEMENT"
        state = "LINEAGE_DISPLACEMENT_PASSED_READY_POSTHOC"
    elif total_tokens > gate["hard_runaway_total_tokens"]:
        decision = "STOP_RESOURCE_RUNAWAY"
        state = "COMPACT_DELTA_RESOURCE_STOP"
    else:
        decision = "REJECT_ADMISSION_LINEAGE_DISPLACEMENT"
        state = "LINEAGE_DISPLACEMENT_REJECTED_STOP"
    commitment = {
        ...
        "lineage_displacement_gate": "PASS" if passed else "REJECT",
        "decision": decision,
    ...
    *, item, replication_id, experimental_arm_id, route_id,
    ...
    return ProviderCognitiveTask(
        ...
        objective=(
            ...
            "opportunity_bindings. These lineage fields are immutable. "
        ...
        allowed_evidence=list(refs),
        expected_schema=lineage_bound_delta_schema(item=item, refs=refs),
        timeout_seconds=min(600, adapter.profile.max_timeout_seconds),
        failure_semantics="preserve_failure",
    )
...
def _runtime_metrics(run, corpus):
    ...
    authorized = {
        key
        for key, value in run["lineage_displacement_receipts"].items()
    ...
    return {
        ...
        "opportunity_receipt_coverage": round(
            len(run["lineage_displacement_receipts"]) / expected, 6
        ),
...
def _validate_preregistration(preregistration, corpus):
    ...
    if (
        ...
        raise ValueError("lineage_displacement_preregistration_invalid")
...
def _validate_run(run, corpus, preregistration):
    ...
    if (
        ...
        raise ValueError("lineage_displacement_run_invalid")
...
def _checkpoint(
    ...
    counter_valid, failures, lineage_displacement_receipts,
    lineage_valid, pairwise_views, pairwise_receipts, pairwise_valid,
):
    if callback is None:
        return
    commitment = {
        "progress_version": "lineage_displacement_progress_v0_47",
        "source_corpus_hash": corpus["artifact_hash"],
        "source_preregistration_hash": preregistration["artifact_hash"],
        ...
        "lineage_displacement_receipts": dict(
            lineage_displacement_receipts
        ),
