"""Blinded typed panel for ternary boundary review v0.84."""
...
from .admission_v9_fresh_holdout import validate_fresh_holdout
from .provider_telemetry import hash_payload


PANEL_VERSION = "admission_v9_ternary_boundary_external_panel_v0_84"
...
def build_external_panel(
    *,
    ...
    if (
        ...
        or evaluation.get("source_run_hashes")
        != {name: run["run_hash"] for name, run in runs.items()}
        or candidate_run.get("source_staged_run_hash")
        != staged_run["run_hash"]
        or evaluation["operational_decision"]
        != "READY_EXTERNAL_TYPED_PANEL"
    ):
        raise ValueError("admission_v9_external_lineage_invalid")
    panel_id = "admission-v9-panel-" + hash_payload([
        PANEL_VERSION,
        panel["artifact_hash"],
        *[runs[name]["run_hash"] for name in sorted(runs)],
        evaluation["artifact_hash"],
    ])[:18]
    units = _public_units(panel)
    packs, lane_bindings = [], {}
    for lane_id, provider, model in LANE_SPECS:
        items, bindings = [], {}
        for unit in units:
            annotation_id = "admission-v9-annotation-" + hash_payload([
                PANEL_VERSION,
                panel_id,
                lane_id,
                unit["case_id"],
        ...
        value = {
            ...
            "rubric": RUBRIC,
            "instructions": (
                "Assess each span independently against the supplied target "
                "object. Do not choose a minimal evidence set. An explicit "
                "comparison between target arms is effect-bearing, including "
                "a null or no-difference result; referring jointly to the two "
                "arms does not by itself pool the outcome. Omission of a "
                "component is not contradiction when the span unambiguously "
                "corefers to the target comparison. A genuinely different "
                "intervention, comparator, outcome, or non-separable composite "
                "must not be admitted as exact-object evidence. Retain context "
                "only when it materially identifies, interprets, scopes, or "
                "validates the exact target. Return exactly one compatible "
                "typed label per annotation_id."
            ),
            "items": items,
    ...
    manifest_value = {
        ...
        "staged_outputs_exposed_to_annotators": False,
...
def validate_external_panel(*, packs, manifest, source_inputs=None):
    ...
    hidden = (
        "candidate_outputs_exposed_to_annotators",
        "staged_outputs_exposed_to_annotators",
        "atomic_outputs_exposed_to_annotators",
        "baseline_outputs_exposed_to_annotators",
        "benchmark_gold_exposed_to_annotators",
    )
    if (
        observed != set(LANE_SPECS)
        or any(manifest.get(field) is not False for field in hidden)
    ):
        raise ValueError("admission_v9_external_manifest_invalid")
    lane_ids = []
    for pack in packs:
        ...
        if (
            ...
            raise ValueError("admission_v9_external_pack_invalid")
        forbidden = (
            "private_gold",
            "gold_rationale",
            "predicted_label",
        ...
        if any(value in str(pack).casefold() for value in forbidden):
            raise ValueError("admission_v9_external_pack_leak")
        lane_ids.append(ids)
    if lane_ids[0] & lane_ids[1]:
        raise ValueError("admission_v9_external_lane_id_collision")
    if source_inputs is not None and (packs, manifest) != (
        build_external_panel(**source_inputs)
    ):
        raise ValueError("admission_v9_external_panel_not_deterministic")
...
def validate_annotation_response(response, *, pack):
    ...
    if (
        ...
        raise ValueError("admission_v9_annotation_response_binding_invalid")
    expected_ids = {
        item["annotation_id"] for item in pack["items"]
    }
    labels = response.get("labels")
    if not isinstance(labels, list) or len(labels) != len(expected_ids):
        raise ValueError("admission_v9_annotation_response_count_invalid")
    observed = []
    for label in labels:
        if (
            ...
            raise ValueError("admission_v9_annotation_label_shape_invalid")
        ...
        if (
            ...
            raise ValueError("admission_v9_annotation_label_invalid")
    if len(observed) != len(set(observed)) or set(observed) != expected_ids:
        raise ValueError("admission_v9_annotation_ids_invalid")
...
def _validate_hash(value, field):
    ...
    if value.get(field) != hash_payload(commitment):
        raise ValueError(f"admission_v9_external_{field}_invalid")
