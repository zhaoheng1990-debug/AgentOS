"""Independent truth-state and prospective null-role experiment v0.40."""
...
RUNTIME_VERSION = "null_role_experiment_v0_40"
ARM_IDS = ("A1_BASELINE", "A2_NULL_ROLE_COMPOSED")


def build_null_role_preregistration(
    *, corpus, prior_stop, prior_closure, cost_analysis
):
    validate_null_role_holdout(corpus)
    if (
        prior_stop.get("decision") != "STOP_CONTRACT_INFEASIBLE"
        or prior_stop.get("candidate_state")
        != "TRUTH_VALUE_CONTRACT_STOP"
        or prior_closure.get("candidate_state")
        != "TRUTH_VALUE_CONTRACT_STOP"
        or prior_stop.get("formal_benchmark_completed") is not False
        or prior_stop.get("private_synthetic_outcomes_used") is not False
        or prior_stop.get(
            "maximum_possible_standard_contract_coverage", 1.0
        ) >= prior_stop.get(
            "frozen_minimum_standard_contract_coverage", 0.0
        )
        or prior_stop.get("contract_failure_reason_counts", {}).get(
            "NON_NULL_DISCRIMINATION_INVALID", 0
        ) <= 0
        or cost_analysis.get("analysis_version")
        != "self_value_experiment_v0_38"
    ):
        raise ValueError("null_role_prior_invalid")
    prior_total = int(cost_analysis["physical_total_tokens"])
    commitment = {
        ...
        "source_null_role_contract_version": CONTRACT_VERSION,
        "source_score_map_hash": hash_payload(SCORE_MAP),
        "source_rank_bonus": list(SOURCE_RANK_BONUS),
        "frozen_hypothesis": (
            "Current relation truth state and the prospective information "
            "role of a future null are independent axes. Allowing every "
            "candidate to express that role, while protecting supported "
            "discriminating nulls from discard, will restore contract "
            "coverage and preserve candidate value without a third "
            "Provider call."
        ),
        "replication_ids": list(REPLICATION_IDS),
        "candidate_count_per_final_receipt": 3,
        "primary_contrast": "A2_NULL_ROLE_COMPOSED_MINUS_A1_BASELINE",
        "success_gate": {
            "minimum_provider_completion_per_required_stage": 0.95,
            "minimum_prompt_identity_coverage": 1.0,
            "minimum_standard_null_role_contract_coverage": 0.95,
            "minimum_counter_null_role_contract_coverage": 0.9,
        ...
        "cost_policy": {
            ...
            "standard_null_role_cost_symmetric_between_arms": True,
    ...
    *, corpus, preregistration, adapter, checkpoint_callback=None
        ...
        if envelope.status != "COMPLETED":
            ...
            if contract_failures:
                failures.append({
                    ...
                    "stage": "STANDARD_NULL_ROLE_CONTRACT",
    ...
    for key, trigger in sorted(trigger_receipts.items()):
        ...
        provisional = raw_receipts[
            f"{rep}:{arm}:STANDARD_NULL_ROLE:{case_id}"
        ]
        task = _null_role_task(
            item=item, replication_id=rep,
        ...
        calls.append(call)
        raw_key = (
            f"{rep}:{arm}:INDEPENDENT_COUNTER_NULL_ROLE:{case_id}"
        )
        if envelope.status != "COMPLETED":
            failures.append(_provider_failure(call))
        else:
            ...
            if contract_failures:
                failures.append({
                    ...
                    "arm_id": arm,
                    "case_id": case_id,
                    "stage": "COUNTER_NULL_ROLE_CONTRACT",
                    "failures": contract_failures,
                ...
                failures.append({
                    ...
                    "failures": ["STANDARD_NULL_ROLE_INVALID"],
                })
            else:
    ...
    commitment = {
        ...
        "standard_null_role_valid_keys": standard_valid,
        "counter_null_role_valid_keys": counter_valid,
        "failures": failures,
...
def analyze_null_role_experiment(*, corpus, preregistration, run):
    validate_null_role_holdout(corpus)
    _validate_preregistration(preregistration, corpus)
    ...
    for rep in REPLICATION_IDS:
        ...
        deltas = {
            item["case_id"]: round(
                totals[(item["case_id"], "A2_NULL_ROLE_COMPOSED")]["net"]
    ...
    conditions = {
        ...
        "minimum_standard_null_role_contract_coverage": (
            runtime["standard_null_role_contract_coverage"]
            >= gate["minimum_standard_null_role_contract_coverage"]
        ),
        "minimum_counter_null_role_contract_coverage": (
            runtime["counter_null_role_contract_coverage"]
            >= gate["minimum_counter_null_role_contract_coverage"]
        ),
        ...
        "minimum_relation_jaccard_relative_to_baseline": (
            relation["A2_NULL_ROLE_COMPOSED"][
    ...
    if passed:
        decision = "PASS_NULL_ROLE_COMPOSITION"
        state = "NULL_ROLE_PASSED_READY_EXTERNAL_PANEL"
    elif total_tokens > gate["hard_runaway_total_tokens"]:
        decision = "STOP_RESOURCE_RUNAWAY"
        state = "NULL_ROLE_RESOURCE_STOP"
    else:
        decision = "REJECT_NULL_ROLE_COMPOSITION"
        state = "NULL_ROLE_REJECTED_STOP"
    commitment = {
        ...
        "null_role_gate": "PASS" if passed else "REJECT",
        "decision": decision,
...
def _scoring_run(run, *, replication_id, provisional=False):
    ...
    case_ids = sorted({
        ...
        and value["stage"] == "STANDARD_NULL_ROLE"
    })
    for case_id in case_ids:
        for arm in ARM_IDS:
            base = _find_call(
                run, replication_id=replication_id, arm_id=arm,
                case_id=case_id, stage="STANDARD_NULL_ROLE",
            )
            path = [base] if base is not None else []
            if arm == "A2_NULL_ROLE_COMPOSED" and not provisional:
                counter = _find_call(
                    run, replication_id=replication_id, arm_id=arm,
                    case_id=case_id,
                    stage="INDEPENDENT_COUNTER_NULL_ROLE",
            ...
            projection = (
                ...
                if arm == "A2_NULL_ROLE_COMPOSED" and provisional
...
def _runtime_metrics(run, corpus):
    ...
    for arm in ARM_IDS:
        values = [
            ...
            if value["stage"] == "STANDARD_NULL_ROLE"
    ...
    counter_calls = [
        value for value in run["task_calls"]
        if value["stage"] == "INDEPENDENT_COUNTER_NULL_ROLE"
    ]
    if triggered:
        completion["INDEPENDENT_COUNTER_NULL_ROLE"] = round(
            sum(value["status"] == "COMPLETED" for value in counter_calls)
            / len(triggered), 6
        )
        counter_contract = round(
            len(run["counter_null_role_valid_keys"])
            / len(triggered), 6
        )
        execution = round(
        ...
        completion["INDEPENDENT_COUNTER_NULL_ROLE"] = 1.0
    ...
    for rep in REPLICATION_IDS:
        for item in corpus["public_surface"]["items"]:
            left = _find_call(
                ...
                case_id=item["case_id"], stage="STANDARD_NULL_ROLE",
            )
            right = _find_call(
                run, replication_id=rep, arm_id="A2_NULL_ROLE_COMPOSED",
                case_id=item["case_id"], stage="STANDARD_NULL_ROLE",
            )
            identity += bool(
                left and right
    ...
    return {
        ...
        "standard_null_role_contract_coverage": round(
            len(run["standard_null_role_valid_keys"])
            / (expected * len(ARM_IDS)), 6
        ),
        "counter_null_role_contract_coverage": counter_contract,
...
def _composition_metrics(
    *, corpus, run, formal_ledgers, provisional_ledgers
    ...
    for rep in REPLICATION_IDS:
        ...
        for item in corpus["public_surface"]["items"]:
            case = item["case_id"]
            key = f"{rep}:A2_NULL_ROLE_COMPOSED:{case}"
            ...
            gross_uplifts.append(
                after[(case, "A2_NULL_ROLE_COMPOSED")]["gross"]
                - before[(case, "A2_NULL_ROLE_COMPOSED")]["gross"]
            )
            net_uplifts.append(
                after[(case, "A2_NULL_ROLE_COMPOSED")]["net"]
                - before[(case, "A2_NULL_ROLE_COMPOSED")]["net"]
            )
            token_penalties.append(
                after[(case, "A2_NULL_ROLE_COMPOSED")]["token_cost"]
                - before[(case, "A2_NULL_ROLE_COMPOSED")]["token_cost"]
            )
    ...
    return {
        ...
        "null_role_axis_distribution": dict(axes),
...
def _validate_preregistration(preregistration, corpus):
    ...
    if (
        ...
        raise ValueError("null_role_preregistration_invalid")
...
def _validate_run(run, corpus, preregistration):
    ...
    if (
        ...
        raise ValueError("null_role_run_invalid")
...
def _checkpoint(
    ...
    if callback is None:
        return
    commitment = {
        "progress_version": "null_role_progress_v0_40",
        "source_corpus_hash": corpus["artifact_hash"],
        "source_preregistration_hash": preregistration["artifact_hash"],
        ...
        "standard_null_role_valid_keys": list(standard_valid),
        "counter_null_role_valid_keys": list(counter_valid),
