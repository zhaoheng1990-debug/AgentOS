"""Build v0.25 state while retaining v0.24 only as diagnostic prior."""
...
from .holdout_v22_benchmark import HOLDOUT_V22_ITEM_FINGERPRINTS, build_holdout_v22_harness
from .holdout_v23_benchmark import HOLDOUT_V23_ITEM_DOMAINS, HOLDOUT_V23_ROUTING_FINGERPRINTS
from .multi_cycle_resolution import MultiCycleResolutionLifecycle
from .problem_formulation_workflow import build_problem_formulation_state
from .structural_operator_policy import build_structural_operator_schedule


_ACTIONS = ("PEER_AGREEMENT_RESOLVED", "PEER_OVERRIDE_RESOLVED",
            "PRIMARY_KEEP_RESOLVED", "CANDIDATE_REVISION_RESOLVED")


def build_problem_dialogue_state(*, v24_report_path, **values):
    prior = build_problem_formulation_state(**values)
    harness = build_holdout_v22_harness()
    resolution = build_resolution_receipt_from_report(
        report_path=v24_report_path, harness=harness,
        item_fingerprints=HOLDOUT_V22_ITEM_FINGERPRINTS,
    ...
    resolutions = (*prior["resolution_evidence_receipts"], resolution)
    contexts = (*prior["context_evidence_receipts"], context)
    resolution_lifecycle, context_lifecycle = (
        MultiCycleResolutionLifecycle(), ContextResolutionLifecycle(),
    )
    resolution_candidates, resolution_promotions = _promote_all(
        resolution_lifecycle, resolutions,
    )
    context_candidates, context_promotions = _promote_all(context_lifecycle, contexts)
