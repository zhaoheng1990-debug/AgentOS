"""Identity-blind GPT/Gemini/Kimi panel for warrant semantics."""
...
PANEL_VERSION = "clarification_warrant_panel_v0_10"
ADJUDICATION_VERSION = "clarification_warrant_adjudication_v0_10"
LANE_SPECS = (
    ("annotation-lane-a", "OpenAI", "GPT-5.6"),
    ("annotation-lane-b", "Google", "Gemini-3.1"),
)
K3_SPEC = ("Moonshot", "Kimi-K3")
CRITERIA = ("EXPLICIT_SELECTION", "PRAGMATIC_PREFERENCE", "ASSESSMENT_COMPLETENESS")
ALLOWED_STATES = {
    "EXPLICIT_SELECTION": ("CANDIDATE_A", "CANDIDATE_B", "NONE", "UNCERTAIN"),
    "PRAGMATIC_PREFERENCE": ("CANDIDATE_A", "CANDIDATE_B", "NONE", "UNCERTAIN"),
    "ASSESSMENT_COMPLETENESS": ("COMPLETE", "INCOMPLETE", "UNCERTAIN"),
}
RUBRIC = {
    "EXPLICIT_SELECTION": (
        "Which candidate, if any, is fixed by the prompt itself? Semantic entailment can count even without exact "
        "word repetition. A conventional default or merely better fit does not. Use NONE when neither is fixed."
    ),
    "PRAGMATIC_PREFERENCE": (
        "If forced to answer without clarification, which candidate is the more natural contextual default? "
        "Use NONE when neither has a meaningful advantage and UNCERTAIN only when you cannot assess the preference."
    ),
    "ASSESSMENT_COMPLETENESS": (
        "Is the available prompt sufficient to complete both judgments above, including a legitimate NONE? "
        "An open task can still have a COMPLETE assessment; INCOMPLETE means the judgment process lacks necessary evidence."
    ),
}


def build_warrant_panel(*, corpus_artifact):
    validate_warrant_holdout_artifact(corpus_artifact)
    source_hash = corpus_artifact["artifact_hash"]
    panel_id = "warrant-panel-" + hash_payload([PANEL_VERSION, source_hash])[:16]
    ...
    for lane_id, provider, model in LANE_SPECS:
        ...
        for case in source_items:
            annotation_id = "warrant-annotation-" + hash_payload([PANEL_VERSION, source_hash, lane_id, case["blind_case_id"]])[:18]
            lane_items.append({
        ...
        commitment = {
            ...
            "rubric": RUBRIC,
            "instructions": (
                "Assess every item independently. Do not infer hidden construction labels or compare with other items. "
                "Separate explicit semantic entailment from pragmatic preference. Task openness does not make an "
                "assessment incomplete. Preserve NONE and UNCERTAIN rather than forcing A or B. Return JSON only."
            ),
            "items": lane_items,
...
def validate_warrant_panel(*, packs, manifest, corpus_artifact=None):
    packs = tuple(packs)
    ...
    if manifest.get("manifest_hash") != hash_payload(commitment):
        raise ValueError("warrant_panel_manifest_hash_invalid")
    ...
    if len(packs) != len(LANE_SPECS) or observed_lanes != expected_lanes:
        raise ValueError("warrant_panel_lane_surface_invalid")
    for pack in packs:
        ...
        if pack.get("pack_hash") != hash_payload(pack_commitment) or pack.get("panel_id") != manifest["panel_id"] or manifest["lane_pack_hashes"].get(pack["lane_id"]) != pack["pack_hash"]:
            raise ValueError("warrant_panel_pack_binding_invalid")
        ids = {item["annotation_id"] for item in pack["items"]}
        if ids != set(manifest["private_lane_bindings"][pack["lane_id"]]):
            raise ValueError("warrant_panel_alias_binding_invalid")
    if corpus_artifact is not None:
        if (packs, manifest) != build_warrant_panel(corpus_artifact=corpus_artifact):
            raise ValueError("warrant_panel_semantics_invalid")
...
def validate_annotation_response(response, *, pack):
    ...
    if not isinstance(response, dict) or set(response) != set(contract["required_top_level"]):
        raise ValueError("warrant_annotation_response_shape_invalid")
    expected = {
        "panel_version": PANEL_VERSION,
    ...
    for label in labels:
        ...
        if set(label["criteria"]) != set(CRITERIA) or any(label["criteria"][criterion] not in ALLOWED_STATES[criterion] for criterion in CRITERIA):
            raise ValueError("warrant_annotation_criteria_invalid")
        if set(label["criterion_notes"]) != set(CRITERIA) or any(not isinstance(value, str) or not value.strip() or len(value) > 600 for value in label["criterion_notes"].values()):
            raise ValueError("warrant_annotation_notes_invalid")
        if set(label["criterion_confidence"]) != set(CRITERIA) or any(not isinstance(value, (int, float)) or isinstance(value, bool) or not 0 <= value <= 1 for value in label["criterion_confidence"].values()):
            raise ValueError("warrant_annotation_confidence_invalid")
    if len(observed) != len(set(observed)) or set(observed) != expected_ids:
        raise ValueError("warrant_annotation_id_binding_invalid")


def build_warrant_adjudication(*, packs, panel_manifest, responses):
    ...
    if set(response_index) != set(pack_index) or len(responses) != len(pack_index):
        raise ValueError("warrant_annotation_lane_responses_invalid")
    ...
    for annotation_id, blind_id in sorted(first_bindings.items(), key=lambda item: item[1]):
        ...
        for criterion in CRITERIA:
            ...
            if len(set(states.values())) == 1:
                ...
                continue
            adjudication_id = "warrant-adjudication-" + hash_payload([ADJUDICATION_VERSION, panel_manifest["panel_id"], blind_id, criterion])[:18]
...
def validate_warrant_adjudication(*, pack, manifest, panel_packs=None, panel_manifest=None, responses=None):
    pack_commitment = {key: value for key, value in pack.items() if key != "pack_hash"}
    ...
    if pack.get("pack_hash") != hash_payload(pack_commitment) or manifest.get("manifest_hash") != hash_payload(manifest_commitment):
        raise ValueError("warrant_adjudication_hash_invalid")
    if manifest.get("adjudication_pack_hash") != pack["pack_hash"] or set(manifest["private_disagreement_bindings"]) != {item["adjudication_id"] for item in pack["items"]} or manifest["total_label_count"] != 24 * len(CRITERIA):
        raise ValueError("warrant_adjudication_binding_invalid")
    supplied = (panel_packs, panel_manifest, responses)
    if any(value is not None for value in supplied):
        if not all(value is not None for value in supplied) or (pack, manifest) != build_warrant_adjudication(packs=panel_packs, panel_manifest=panel_manifest, responses=responses):
            raise ValueError("warrant_adjudication_semantics_invalid")
...
def adjudication_response_contract():
    return {
        ...
        "required_blinding_attestation": {"pack_only_context": True, "annotator_identity_unavailable": True, "source_identity_unavailable": True, "prior_scores_unavailable": True, "external_pairing_not_used": True},
    }
...
def validate_adjudication_response(response, *, pack):
    ...
    if not isinstance(response, dict) or set(response) != set(contract["required_top_level"]):
        raise ValueError("warrant_adjudication_response_shape_invalid")
    ...
    if any(response.get(key) != value for key, value in expected.items()) or response.get("blinding_attestation") != contract["required_blinding_attestation"] or not isinstance(response.get("adjudication_session_ref"), str) or not response["adjudication_session_ref"].strip():
        raise ValueError("warrant_adjudication_response_binding_invalid")
    ...
    if not isinstance(decisions, list) or len(decisions) != len(item_index):
        raise ValueError("warrant_adjudication_decision_count_invalid")
    observed = []
    for decision in decisions:
        if not isinstance(decision, dict) or set(decision) != set(contract["required_decision_fields"]):
            raise ValueError("warrant_adjudication_decision_shape_invalid")
        ...
        if not item or decision["selected_state"] not in ALLOWED_STATES[item["criterion"]] or decision["decision_basis"] not in contract["allowed_bases"] or not isinstance(decision["rationale"], str) or not decision["rationale"].strip():
            raise ValueError("warrant_adjudication_decision_invalid")
        confidence = decision["confidence"]
        if not isinstance(confidence, (int, float)) or isinstance(confidence, bool) or not 0 <= confidence <= 1:
            raise ValueError("warrant_adjudication_confidence_invalid")
        ...
        if decision["decision_basis"] in positions and decision["selected_state"] != positions[decision["decision_basis"]]:
            raise ValueError("warrant_adjudication_position_state_invalid")
        if decision["decision_basis"] == "UNRESOLVED" and decision["selected_state"] != "UNCERTAIN":
            raise ValueError("warrant_adjudication_unresolved_state_invalid")
    if len(observed) != len(set(observed)) or set(observed) != set(item_index):
        raise ValueError("warrant_adjudication_id_binding_invalid")


def build_warrant_reference(*, adjudication_pack, adjudication_manifest, response=None):
    validate_warrant_adjudication(pack=adjudication_pack, manifest=adjudication_manifest)
    if adjudication_pack["items"]:
        if response is None:
            raise ValueError("warrant_adjudication_response_required")
    ...
    if set(cells) != {(blind_id, criterion) for blind_id in blind_ids for criterion in CRITERIA}:
        raise ValueError("warrant_reference_surface_incomplete")
    ...
    commitment = {
        ...
        "candidate_state": "WARRANT_MODEL_PANEL_REFERENCE_CANDIDATE",
        "ground_truth_claim": False,
...
def validate_warrant_reference(artifact, *, adjudication_pack=None, adjudication_manifest=None, response=None):
    ...
    for label in labels:
        ...
        if set(label["criterion_sources"]) != set(CRITERIA) or any(source not in ("GPT_GEMINI_AGREEMENT", "KIMI_K3_ADJUDICATION") for source in label["criterion_sources"].values()):
            raise ValueError("warrant_reference_source_invalid")
    if len(ids) != len(set(ids)):
        raise ValueError("warrant_reference_id_invalid")
    supplied = (adjudication_pack, adjudication_manifest)
    if any(value is not None for value in supplied):
        if not all(value is not None for value in supplied) or artifact != build_warrant_reference(adjudication_pack=adjudication_pack, adjudication_manifest=adjudication_manifest, response=response):
            raise ValueError("warrant_reference_semantics_invalid")
...
def _annotation_for(manifest, lane, blind_id):
    ...
    if len(matches) != 1:
        raise ValueError("warrant_annotation_reverse_binding_invalid")
    return matches[0]
