"""Selective single-delta counterproposal experiment v0.41."""
...
from .selective_delta_holdout import (
    REPLICATION_IDS,
...
RUNTIME_VERSION = "selective_delta_experiment_v0_41"
ARM_IDS = ("A1_BASELINE", "A2_SELECTIVE_DELTA")


def build_selective_delta_preregistration(
    *, corpus, prior_analysis, prior_closure, prior_posthoc
):
    validate_selective_delta_holdout(corpus)
    if (
        prior_analysis.get("decision")
        != "REJECT_NULL_ROLE_COMPOSITION"
        or prior_analysis.get("candidate_state")
        != "NULL_ROLE_REJECTED_STOP"
        ...
        or prior_analysis.get("composition_metrics", {}).get(
            "mean_added_token_penalty_cbit", 0.0
        ) <= 1.0
        ...
        or prior_posthoc.get("positive_oracle_cell_count", 0) < 5
    ):
        raise ValueError("selective_delta_prior_invalid")
    ...
    commitment = {
        ...
        "frozen_hypothesis": (
            "A zero-token semantic-asymmetry qualifier can suppress "
            "low-opportunity counterproposal calls, while a single "
            "evidence-bounded delta candidate can preserve most gross Cbit "
            "at materially lower token cost than a three-candidate receipt."
        ),
        "replication_ids": list(REPLICATION_IDS),
        "candidate_count_per_final_receipt": 3,
        "primary_contrast": "A2_SELECTIVE_DELTA_MINUS_A1_BASELINE",
        "success_gate": {
            "minimum_provider_completion_per_required_stage": 0.95,
            "minimum_prompt_identity_coverage": 1.0,
            "minimum_standard_selective_delta_contract_coverage": 0.95,
            "minimum_counter_selective_delta_contract_coverage": 0.9,
        ...
        "qualification_thresholds_frozen_from_v0_40_development": True,
...
def run_selective_delta_experiment(
    *, corpus, preregistration, adapter, checkpoint_callback=None
    ...
    delta_views, candidate_pools, composition_receipts = {}, {}, {}
    ...
    for rep, arm, canonical in matrix:
        ...
        task = _standard_task(
            ...
            stage="STANDARD_SELECTIVE_DELTA", refs=refs, adapter=adapter,
        )
        envelope = ProviderTaskRouter([adapter]).route(task)
        call = _call(
            replication_id=rep, arm_id=arm, case_id=item["case_id"],
            stage="STANDARD_SELECTIVE_DELTA", route_id="A1_ONTOLOGY",
            task=task, envelope=envelope,
        )
        calls.append(call)
        key = f"{rep}:{arm}:{item['case_id']}"
        raw_key = f"{rep}:{arm}:STANDARD_SELECTIVE_DELTA:{item['case_id']}"
        if envelope.status != "COMPLETED":
            ...
            if contract_failures:
                failures.append({
                    ...
                    "stage": "STANDARD_SELECTIVE_DELTA_CONTRACT",
        ...
        _checkpoint(
            ...
            candidate_pools, composition_receipts,
    ...
    for key, qualification in sorted(
        ...
        calls.append(call)
        raw_key = (
            f"{rep}:{arm}:INDEPENDENT_COUNTER_SELECTIVE_DELTA:{case_id}"
        )
        if envelope.status != "COMPLETED":
            failures.append(_provider_failure(call))
        else:
            ...
            contract_failures = validate_single_delta(
                raw_receipt=counter,
                item=delta_view,
                refs=refs,
                excluded_relation_ids=delta_view[
            ...
            if contract_failures:
                failures.append({
                    ...
                    "arm_id": arm,
                    "case_id": case_id,
                    "stage": "COUNTER_SELECTIVE_DELTA_CONTRACT",
                    "failures": contract_failures,
                ...
                failures.append({
                    ...
                    "failures": ["STANDARD_SELECTIVE_DELTA_INVALID"],
                })
            else:
                ...
                final_raw, composition = compose_null_role_valued_pool(
    ...
    commitment = {
        ...
        "standard_selective_delta_valid_keys": standard_valid,
        "counter_selective_delta_valid_keys": counter_valid,
        "failures": failures,
...
def analyze_selective_delta_experiment(*, corpus, preregistration, run):
    validate_selective_delta_holdout(corpus)
    _validate_preregistration(preregistration, corpus)
    ...
    for rep in REPLICATION_IDS:
        ...
        deltas = {
            item["case_id"]: round(
                totals[(item["case_id"], "A2_SELECTIVE_DELTA")]["net"]
    ...
    conditions = {
        ...
        "minimum_standard_selective_delta_contract_coverage": (
            runtime["standard_selective_delta_contract_coverage"]
            >= gate["minimum_standard_selective_delta_contract_coverage"]
        ),
        "minimum_counter_selective_delta_contract_coverage": (
            runtime["counter_selective_delta_contract_coverage"]
            >= gate["minimum_counter_selective_delta_contract_coverage"]
        ),
        ...
        "selective_delta_boundary_preserved": (
    ...
    if passed:
        decision = "PASS_SELECTIVE_DELTA_COMPOSITION"
        state = "SELECTIVE_DELTA_PASSED_READY_EXTERNAL_PANEL"
    elif total_tokens > gate["hard_runaway_total_tokens"]:
        decision = "STOP_RESOURCE_RUNAWAY"
        state = "SELECTIVE_DELTA_RESOURCE_STOP"
    else:
        decision = "REJECT_SELECTIVE_DELTA_COMPOSITION"
        state = "SELECTIVE_DELTA_REJECTED_STOP"
    commitment = {
        ...
        "selective_delta_gate": "PASS" if passed else "REJECT",
        "decision": decision,
...
def _delta_task(
    *, item, replication_id, experimental_arm_id, route_id,
    ...
    return ProviderCognitiveTask(
        ...
        task_kind=TASK_KIND,
        objective=(
            f"{prompts[route_id]} Return exactly one falsifiable candidate "
            "whose source-target relation is not in excluded_relation_ids. "
            "Use only the supplied evidence subset. Record current "
            ...
            "evidence spans. Do not reconstruct the base candidate set or "
            "an object census. The numeric Runtime score map and private "
        ...
        inputs={
            ...
            "receipt_arm_id": "A1_ONTOLOGY",
            "delta_case": item,
            "excluded_relation_ids": item["excluded_relation_ids"],
        ...
        allowed_evidence=list(refs),
        expected_schema=single_delta_schema(item=item, refs=refs),
        timeout_seconds=min(600, adapter.profile.max_timeout_seconds),
        failure_semantics="preserve_failure",
    )
...
def _scoring_run(run, *, replication_id, provisional=False):
    ...
    case_ids = sorted({
        ...
        and value["stage"] == "STANDARD_SELECTIVE_DELTA"
    })
    for case_id in case_ids:
        for arm in ARM_IDS:
            base = _find_call(
                run, replication_id=replication_id, arm_id=arm,
                case_id=case_id, stage="STANDARD_SELECTIVE_DELTA",
            )
            path = [base] if base is not None else []
            if arm == "A2_SELECTIVE_DELTA" and not provisional:
                counter = _find_call(
                    run, replication_id=replication_id, arm_id=arm,
                    case_id=case_id,
                    stage="INDEPENDENT_COUNTER_SELECTIVE_DELTA",
            ...
            projection = (
                ...
                if arm == "A2_SELECTIVE_DELTA" and provisional
...
def _runtime_metrics(run, corpus):
    ...
    for arm in ARM_IDS:
        values = [
            ...
            if value["stage"] == "STANDARD_SELECTIVE_DELTA"
    ...
    counter_calls = [
        value for value in run["task_calls"]
        if value["stage"] == "INDEPENDENT_COUNTER_SELECTIVE_DELTA"
    ]
    if qualified:
        completion["INDEPENDENT_COUNTER_SELECTIVE_DELTA"] = round(
            sum(value["status"] == "COMPLETED" for value in counter_calls)
            / len(qualified), 6
        )
        counter_contract = round(
            len(run["counter_selective_delta_valid_keys"])
            / len(qualified), 6
        )
        execution = round(
            len(run["composition_receipts"]) / len(qualified), 6
        )
    else:
        completion["INDEPENDENT_COUNTER_SELECTIVE_DELTA"] = 1.0
    ...
    for rep in REPLICATION_IDS:
        for item in corpus["public_surface"]["items"]:
            left = _find_call(
                ...
                case_id=item["case_id"], stage="STANDARD_SELECTIVE_DELTA",
            )
            right = _find_call(
                run, replication_id=rep, arm_id="A2_SELECTIVE_DELTA",
                case_id=item["case_id"], stage="STANDARD_SELECTIVE_DELTA",
            )
            identity += bool(
                left and right
    ...
    return {
        ...
        "standard_selective_delta_contract_coverage": round(
            len(run["standard_selective_delta_valid_keys"])
            / (expected * len(ARM_IDS)), 6
        ),
        "counter_selective_delta_contract_coverage": counter_contract,
...
def _composition_metrics(
    *, corpus, run, formal_ledgers, provisional_ledgers
    ...
    for rep in REPLICATION_IDS:
        ...
        for item in corpus["public_surface"]["items"]:
            case = item["case_id"]
            key = f"{rep}:A2_SELECTIVE_DELTA:{case}"
            ...
            gross_uplifts.append(
                after[(case, "A2_SELECTIVE_DELTA")]["gross"]
                - before[(case, "A2_SELECTIVE_DELTA")]["gross"]
            )
            net_uplifts.append(
                after[(case, "A2_SELECTIVE_DELTA")]["net"]
                - before[(case, "A2_SELECTIVE_DELTA")]["net"]
            )
            token_penalties.append(
                after[(case, "A2_SELECTIVE_DELTA")]["token_cost"]
                - before[(case, "A2_SELECTIVE_DELTA")]["token_cost"]
            )
    ...
    return {
        ...
        "selective_delta_axis_distribution": dict(axes),
        "triggered_composition_gross_uplift": (
...
def _validate_preregistration(preregistration, corpus):
    ...
    if (
        ...
        raise ValueError("selective_delta_preregistration_invalid")
...
def _validate_run(run, corpus, preregistration):
    ...
    if (
        ...
        raise ValueError("selective_delta_run_invalid")


def _checkpoint(
    callback, corpus, preregistration, calls, raw_receipts,
    provisional_projections, final_projections, trigger_receipts,
    qualification_receipts, delta_views, candidate_pools,
    composition_receipts, standard_valid, counter_valid, failures,
):
    if callback is None:
        return
    commitment = {
        "progress_version": "selective_delta_progress_v0_41",
        "source_corpus_hash": corpus["artifact_hash"],
        "source_preregistration_hash": preregistration["artifact_hash"],
        ...
        "standard_selective_delta_valid_keys": list(standard_valid),
        "counter_selective_delta_valid_keys": list(counter_valid),
