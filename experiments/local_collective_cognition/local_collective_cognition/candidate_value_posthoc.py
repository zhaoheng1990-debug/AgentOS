"""Private-outcome oracle-capture audit for completed v0.37 evidence."""
...
from .candidate_value_experiment import RUNTIME_VERSION
from .candidate_value_holdout import REPLICATION_IDS
...
POSTHOC_VERSION = "candidate_value_posthoc_v0_37"


def analyze_candidate_value_posthoc(*, corpus, run, analysis):
    counter_uplifts, selected_uplifts, oracle_uplifts = [], [], []
    regrets, capture = [], []
    selected_outcomes, verdict_outcomes = Counter(), Counter()
    cells = []
    items = {
        value["case_id"]: value
    ...
    for key, composition in sorted(run["composition_receipts"].items()):
        ...
        if rep not in REPLICATION_IDS or arm != "A2_VALUE_COMPOSED":
            continue
        item = items[case_id]
        ...
        for pool_id, scored in scored_by_pool.items():
            outcome = by_id[pool_id]["outcome_state"]
            verdict = scored["provider_evaluation"]["verdict"]
            relation_state = scored["provider_evaluation"][
                "relation_evidence"
            ]
            verdict_outcomes[(verdict, relation_state, outcome)] += 1
    ...
    commitment = {
        ...
        "value_judgment_outcome_distribution": {
            f"{verdict}:{relation}:{outcome}": count
            for (verdict, relation, outcome), count in sorted(
                verdict_outcomes.items()
            )
        },
        "cells": cells,
