"""Blinded typed-evidence panel for atomic witness v0.80."""
...
from .admission_v5_fresh_holdout import validate_fresh_holdout
from .provider_telemetry import hash_payload


PANEL_VERSION = "admission_v5_atomic_typed_external_panel_v0_80"


def build_external_panel(
    *, panel, baseline_run, candidate_run, evaluation
):
    validate_fresh_holdout(panel)
    _validate_hash(baseline_run, "run_hash")
    _validate_hash(candidate_run, "run_hash")
    _validate_hash(evaluation, "artifact_hash")
    if (
        baseline_run["source_panel_hash"] != panel["artifact_hash"]
        or candidate_run["source_panel_hash"] != panel["artifact_hash"]
        or evaluation["source_panel_hash"] != panel["artifact_hash"]
        or evaluation["source_baseline_run_hash"]
        != baseline_run["run_hash"]
        or evaluation["source_candidate_run_hash"]
        != candidate_run["run_hash"]
        or evaluation["operational_decision"]
        != "READY_EXTERNAL_TYPED_PANEL"
    ):
        raise ValueError("admission_v5_external_lineage_invalid")
    panel_id = "admission-v5-panel-" + hash_payload([
        PANEL_VERSION,
        panel["artifact_hash"],
        baseline_run["run_hash"],
        candidate_run["run_hash"],
    ...
    for lane_id, provider, model in LANE_SPECS:
        ...
        for unit in public_units:
            annotation_id = "admission-v5-annotation-" + hash_payload([
                PANEL_VERSION,
                panel_id,
                lane_id,
                unit["case_id"],
        ...
        value = {
            ...
            "instructions": (
                ...
                "uncertainty, significance, and quantitative corroboration "
                "can all be effect-bearing. Return exactly one compatible "
    ...
    manifest_value = {
        ...
        "frozen_candidate_run_hash": candidate_run["run_hash"],
...
def validate_external_panel(*, packs, manifest, source_inputs=None):
    ...
    if (
        ...
        raise ValueError("admission_v5_external_manifest_invalid")
    lane_ids = []
    for pack in packs:
        ...
        if (
            ...
            raise ValueError("admission_v5_external_pack_invalid")
        forbidden = (
            "private_gold",
            "gold_rationale",
            "predicted_label",
        ...
        if any(value in str(pack).casefold() for value in forbidden):
            raise ValueError("admission_v5_external_pack_leak")
        lane_ids.append(ids)
    if lane_ids[0] & lane_ids[1]:
        raise ValueError("admission_v5_external_lane_id_collision")
    if source_inputs is not None and (packs, manifest) != (
        build_external_panel(**source_inputs)
    ):
        raise ValueError("admission_v5_external_panel_not_deterministic")
...
def validate_annotation_response(response, *, pack):
    ...
    if (
        ...
        raise ValueError("admission_v5_annotation_response_binding_invalid")
    expected_ids = {
        item["annotation_id"] for item in pack["items"]
    }
    labels = response.get("labels")
    if not isinstance(labels, list) or len(labels) != len(expected_ids):
        raise ValueError("admission_v5_annotation_response_count_invalid")
    observed = []
    for label in labels:
        if (
            ...
            raise ValueError("admission_v5_annotation_label_shape_invalid")
        ...
        if (
            ...
            raise ValueError("admission_v5_annotation_label_invalid")
    if len(observed) != len(set(observed)) or set(observed) != expected_ids:
        raise ValueError("admission_v5_annotation_ids_invalid")
...
def _validate_hash(value, field):
    ...
    if value.get(field) != hash_payload(commitment):
        raise ValueError(f"admission_v5_external_{field}_invalid")
