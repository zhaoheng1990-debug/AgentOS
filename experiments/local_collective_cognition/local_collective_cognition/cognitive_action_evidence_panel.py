"""Blinded external panel for evidence-state calibration v0.19."""
...
EVIDENCE_PANEL_VERSION = "cognitive_action_evidence_external_panel_v0_19"
EVIDENCE_ADJUDICATION_VERSION = "cognitive_action_evidence_external_adjudication_v0_19"
LANE_SPECS = (
...
def build_evidence_external_panel(*, corpus, run):
    validate_evidence_holdout(corpus)
    run_commitment = {key: value for key, value in run.items() if key != "run_hash"}
    if (
        run.get("run_hash") != hash_payload(run_commitment)
        or run.get("source_corpus_hash") != corpus["artifact_hash"]
        or run.get("candidate_outputs_frozen_before_external_reference") is not True
    ):
        raise ValueError("evidence_panel_run_invalid")
    panel_id = "evidence-panel-" + hash_payload([
        EVIDENCE_PANEL_VERSION, corpus["artifact_hash"], run["run_hash"]
    ])[:16]
    packs, bindings = [], {}
    for lane_id, provider, model in LANE_SPECS:
        items, lane_bindings = [], {}
        for source in corpus["public_surface"]["items"]:
            annotation_id = "evidence-annotation-" + hash_payload([
                EVIDENCE_PANEL_VERSION, panel_id, lane_id, source["conflict_id"]
            ])[:18]
            items.append({
        ...
        commitment = {
            "panel_version": EVIDENCE_PANEL_VERSION,
            "panel_id": panel_id,
            "lane_id": lane_id,
            ...
            "criterion_definitions": {
                "SELECTED_OBJECT": "Which candidate, if any, is semantically fixed by the request.",
                "SELECTION_BASIS": "Whether selection is lexical, compositional, pragmatic, unsupported, or uncertain.",
                "PRAGMATIC_PREFERENCE": "Which candidate context favors when neither is entailed.",
                ...
                "ASSESSMENT_PROCESS_STATE": "Whether assessment completed; a justified open or opaque conclusion can be COMPLETE.",
            },
            "instructions": (
                "Assess each object independently. Candidate option text is not itself definition evidence. Distinguish an explicit "
                "request definition or unambiguous established term from a result derived by composing constraints. Keep soft "
                "ambiguity separate from a missing external codebook or specification. Return one coherent five-axis tuple per item."
            ),
            "items": items,
    ...
    manifest_commitment = {
        ...
        "current_phase": "AWAITING_GPT_GEMINI_EVIDENCE_STATE_TUPLES",
...
def validate_evidence_external_panel(*, packs, manifest, source_inputs=None):
    packs = tuple(packs)
    ...
    if (
        ...
        raise ValueError("evidence_panel_manifest_invalid")
    ids_by_lane = []
    for pack in packs:
        ...
        if (
            ...
            or ids != set(manifest["private_lane_bindings"][pack["lane_id"]])
        ):
            raise ValueError("evidence_panel_pack_invalid")
        forbidden = ("deepseek-r1", "definition_source_receipt", "free_label_control", "contrastive_source_calibrator")
        if any(value in str(pack).casefold() for value in forbidden):
            raise ValueError("evidence_panel_candidate_leak")
        ids_by_lane.append(ids)
    if ids_by_lane[0] & ids_by_lane[1]:
        raise ValueError("evidence_panel_lane_alias_collision")
    if source_inputs is not None and (packs, manifest) != build_evidence_external_panel(**source_inputs):
        raise ValueError("evidence_panel_semantics_invalid")
...
def validate_evidence_annotation_response(response, *, pack):
    ...
    if (
        ...
        raise ValueError("evidence_annotation_response_binding_invalid")
    expected_ids = {item["annotation_id"] for item in pack["items"]}
    labels = response.get("labels")
    if not isinstance(labels, list) or len(labels) != len(expected_ids):
        raise ValueError("evidence_annotation_response_count_invalid")
    observed = []
    for label in labels:
        observed.append(label.get("annotation_id") if isinstance(label, dict) else None)
        if not isinstance(label, dict) or set(label) != set(contract["required_label_fields"]):
            raise ValueError("evidence_annotation_label_shape_invalid")
        ...
        if (
            ...
            or len(label["tuple_rationale"]) > 1200
        ):
            raise ValueError("evidence_annotation_tuple_invalid")
    if len(observed) != len(set(observed)) or set(observed) != expected_ids:
        raise ValueError("evidence_annotation_ids_invalid")


def build_evidence_external_adjudication(*, packs, panel_manifest, responses, corpus):
    ...
    if len(response_index) != len(responses) or set(response_index) != set(pack_index):
        raise ValueError("evidence_annotation_lanes_invalid")
    for lane, response in response_index.items():
        validate_evidence_annotation_response(response, pack=pack_index[lane])
    lane_labels = {
        lane: {
    ...
    for conflict_id in sorted(public):
        ...
        if len({hash_payload(value) for value in tuples.values()}) == 1:
            ...
            continue
        adjudication_id = "evidence-adjudication-" + hash_payload([
            EVIDENCE_ADJUDICATION_VERSION, panel_manifest["panel_id"], conflict_id
        ])[:18]
        ...
        bindings[adjudication_id] = {
            "conflict_id": conflict_id,
            "position_lanes": {f"POSITION_{index}": lane for index, lane in enumerate(ordered, start=1)},
            "lane_annotation_ids": annotation_ids,
        }
    items.sort(key=lambda item: hash_payload([panel_manifest["panel_id"], item["adjudication_id"]]))
    pack_commitment = {
        "panel_version": EVIDENCE_PANEL_VERSION,
        "adjudication_version": EVIDENCE_ADJUDICATION_VERSION,
        ...
        "prior_scores": "WITHHELD",
        "instructions": (
            "Adjudicate each disagreement as one complete five-axis tuple. Candidate option text is not definition evidence. "
            "Distinguish explicit or established definitions, composed constraints, soft ambiguity, and missing external "
            "specifications. Select one anonymous full position or independently reassess the whole tuple."
        ),
        "items": items,
    ...
    manifest_commitment = {
        ...
        "reference_state": "AWAITING_KIMI_K3_EVIDENCE_FULL_TUPLE_ADJUDICATION" if items else "REFERENCE_READY_FROM_FULL_TUPLE_AGREEMENT",
...
def validate_evidence_external_adjudication(*, pack, manifest, source_inputs=None):
    pack_commitment = {key: value for key, value in pack.items() if key != "pack_hash"}
    ...
    if (
        ...
        raise ValueError("evidence_external_adjudication_invalid")
    forbidden = ("gpt-5.6", "gemini-3.1", "openai", "google", "deepseek-r1")
    if any(value in str(pack).casefold() for value in forbidden):
        raise ValueError("evidence_adjudication_identity_leak")
    for item in pack["items"]:
        if {position["position_id"] for position in item["anonymous_full_tuple_positions"]} != {"POSITION_1", "POSITION_2"}:
            raise ValueError("evidence_adjudication_positions_invalid")
    if source_inputs is not None and (pack, manifest) != build_evidence_external_adjudication(**source_inputs):
        raise ValueError("evidence_adjudication_semantics_invalid")
...
def validate_evidence_adjudication_response(response, *, pack):
    ...
    if (
        ...
        raise ValueError("evidence_adjudication_response_binding_invalid")
    ...
    if not isinstance(decisions, list) or len(decisions) != len(item_index):
        raise ValueError("evidence_adjudication_response_count_invalid")
    observed = []
    for decision in decisions:
        if not isinstance(decision, dict) or set(decision) != set(contract["required_decision_fields"]):
            raise ValueError("evidence_adjudication_decision_shape_invalid")
        ...
        if (
            ...
            raise ValueError("evidence_adjudication_decision_invalid")
        positions = {
        ...
        if decision["decision_basis"] in positions and criteria != positions[decision["decision_basis"]]:
            raise ValueError("evidence_adjudication_position_binding_invalid")
    if len(observed) != len(set(observed)) or set(observed) != set(item_index):
        raise ValueError("evidence_adjudication_ids_invalid")
