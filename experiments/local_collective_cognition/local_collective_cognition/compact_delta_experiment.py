"""Compact semantic-delta counterproposal experiment v0.42."""
...
from .compact_delta_holdout import (
    REPLICATION_IDS,
    validate_compact_delta_holdout,
)
...
RUNTIME_VERSION = "compact_delta_experiment_v0_42"
ARM_IDS = ("A1_BASELINE", "A2_COMPACT_DELTA")


def build_compact_delta_preregistration(
    *, corpus, prior_analysis, prior_closure, prior_posthoc
):
    validate_compact_delta_holdout(corpus)
    if (
        prior_analysis.get("decision")
        != "REJECT_SELECTIVE_DELTA_COMPOSITION"
        or prior_analysis.get("candidate_state")
        != "SELECTIVE_DELTA_REJECTED_STOP"
        ...
        or prior_posthoc.get("invalid_delta_contract_count") != 2
    ):
        raise ValueError("compact_delta_prior_invalid")
    ...
    commitment = {
        ...
        "frozen_hypothesis": (
            "Structured excluded object pairs plus a compact semantic-only "
            "Provider contract will eliminate relation duplication and "
            "reduce mean delta token penalty below 0.6 Cbit while preserving "
            "the frozen qualifier and deterministic composition."
        ),
...
def run_compact_delta_experiment(
...
def analyze_compact_delta_experiment(*, corpus, preregistration, run):
    validate_compact_delta_holdout(corpus)
    _validate_preregistration(preregistration, corpus)
    ...
    if passed:
        decision = "PASS_COMPACT_DELTA_COMPOSITION"
        state = "COMPACT_DELTA_PASSED_READY_EXTERNAL_PANEL"
        ...
        state = "COMPACT_DELTA_REJECTED_STOP"
    commitment = {
        ...
        "compact_delta_gate": "PASS" if passed else "REJECT",
        "decision": decision,
...
def _validate_preregistration(preregistration, corpus):
    ...
    if (
        ...
        raise ValueError("compact_delta_preregistration_invalid")
...
def _validate_run(run, corpus, preregistration):
    ...
    if (
        ...
        raise ValueError("compact_delta_run_invalid")
...
def _checkpoint(
    ...
    if callback is None:
        return
    commitment = {
        "progress_version": "compact_delta_progress_v0_42",
        "source_corpus_hash": corpus["artifact_hash"],
        "source_preregistration_hash": preregistration["artifact_hash"],
