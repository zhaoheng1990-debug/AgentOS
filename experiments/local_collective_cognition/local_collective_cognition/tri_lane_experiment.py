"""Three-lane post-provider replacement experiment v0.44."""
...
from .tri_lane_holdout import (
    REPLICATION_IDS,
    validate_tri_lane_holdout,
)
...
RUNTIME_VERSION = "tri_lane_experiment_v0_44"
ARM_IDS = ("A1_BASELINE", "A2_COMPACT_DELTA")


def build_tri_lane_preregistration(
    *, corpus, prior_analysis, prior_closure, prior_posthoc
):
    validate_tri_lane_holdout(corpus)
    if (
        prior_analysis.get("decision")
        != "REJECT_REPLACEMENT_SAFETY_GATE"
        or prior_analysis.get("candidate_state")
        != "REPLACEMENT_SAFETY_REJECTED_STOP"
        ...
        or prior_posthoc.get(
            "classification_distribution", {}
        ).get("BENEFICIAL_REJECTED") != 2
    ):
        raise ValueError("tri_lane_prior_invalid")
    ...
    commitment = {
        ...
        "frozen_hypothesis": (
            "A three-lane zero-call gate will eliminate gross harmful "
            "replacement without suppressing informative nulls. Quarantine "
            "replacement remains mechanical; strong focal effects require "
            "the same relation in another replication; informative nulls "
            "use an independent null-value lane; dropped SUPPORTED_NULL "
            "candidates remain protected."
        ),
...
def analyze_tri_lane_experiment(*, corpus, preregistration, run):
    validate_tri_lane_holdout(corpus)
    _validate_preregistration(preregistration, corpus)
    ...
    if passed:
        decision = "PASS_TRI_LANE_REPLACEMENT_GATE"
        state = "TRI_LANE_PASSED_READY_POSTHOC"
    elif total_tokens > gate["hard_runaway_total_tokens"]:
        decision = "STOP_RESOURCE_RUNAWAY"
        state = "COMPACT_DELTA_RESOURCE_STOP"
    else:
        decision = "REJECT_TRI_LANE_REPLACEMENT_GATE"
        state = "TRI_LANE_REJECTED_STOP"
    commitment = {
        ...
        "tri_lane_gate": "PASS" if passed else "REJECT",
        "decision": decision,
...
def _validate_preregistration(preregistration, corpus):
    ...
    if (
        ...
        raise ValueError("tri_lane_preregistration_invalid")
...
def _validate_run(run, corpus, preregistration):
    ...
    if (
        ...
        raise ValueError("tri_lane_run_invalid")
...
def _checkpoint(
    ...
    if callback is None:
        return
    commitment = {
        "progress_version": "tri_lane_progress_v0_44",
        "source_corpus_hash": corpus["artifact_hash"],
        "source_preregistration_hash": preregistration["artifact_hash"],
