...
from .opportunity_routing_holdout import (
    REPLICATION_IDS,
...
def build_opportunity_routing_preregistration(
    *, corpus, prior_analysis, prior_closure, prior_posthoc
    ...
    if (
        ...
        or prior_posthoc.get("positive_oracle_cell_count") != 4
        or prior_posthoc.get(
            "classification_distribution", {}
        ).get("BENEFICIAL_ACCEPTED") != 1
        or prior_posthoc.get(
            "classification_distribution", {}
        ).get("BENEFICIAL_REJECTED") != 3
    ):
        raise ValueError("opportunity_routing_prior_invalid")
    ...
    commitment = {
        ...
        "frozen_hypothesis": (
            "A small Provider-supported admission opportunity set embedded "
            "in the shared standard receipt will let Runtime skip low-value "
            "delta calls before invocation and constrain authorized delta "
            "calls to admission-ready relations. The frozen tri-lane gate "
            "must still prevent harmful promotions, while called deltas "
            "should have materially higher proposal precision than v0.45."
        ),
        ...
        "success_gate": {
            ...
            "minimum_authorized_delta_call_count": 2,
        ...
        "cost_policy": {
            ...
            "soft_expected_total_tokens": math.ceil(prior_total * 1.15),
            "hard_runaway_total_tokens": math.ceil(prior_total * 1.4),
        },
        ...
        "opportunity_routing_adds_provider_calls": False,
...
def run_opportunity_routing_experiment(
    *, corpus, preregistration, adapter, checkpoint_callback=None
    ...
    for key, qualification in sorted(
        ...
        opportunity_routing_receipts[key] = (
            derive_admission_opportunity_receipt(
                key=key,
    ...
    for key, qualification in sorted(
        ...
        routing = opportunity_routing_receipts[key]
        ...
        if envelope.status != "COMPLETED":
            ...
            contract_failures = validate_opportunity_delta(
                delta=delta,
                item=delta_view,
                refs=refs,
            )
    ...
    commitment = {
        ...
        "opportunity_routing_receipts": opportunity_routing_receipts,
...
def analyze_opportunity_routing_experiment(*, corpus, preregistration, run):
    validate_opportunity_routing_holdout(corpus)
    _validate_preregistration(preregistration, corpus)
    ...
    conditions = {
        ...
        "compact_delta_boundary_preserved": (
            ...
            and run["opportunity_routing_adds_provider_calls"] is False
            and all(
    ...
    if passed:
        decision = "PASS_ADMISSION_OPPORTUNITY_ROUTING"
        state = "OPPORTUNITY_ROUTING_PASSED_READY_POSTHOC"
        ...
        state = "OPPORTUNITY_ROUTING_REJECTED_STOP"
    commitment = {
        ...
        "opportunity_routing_gate": "PASS" if passed else "REJECT",
        "decision": decision,
...
def _delta_task(
    *, item, replication_id, experimental_arm_id, route_id,
    ...
    return ProviderCognitiveTask(
        ...
        allowed_evidence=list(refs),
        expected_schema=opportunity_delta_schema(item=item, refs=refs),
        timeout_seconds=min(600, adapter.profile.max_timeout_seconds),
        failure_semantics="preserve_failure",
    )
...
def _runtime_metrics(run, corpus):
    ...
    authorized = {
        key
        for key, value in run["opportunity_routing_receipts"].items()
    ...
    if authorized:
        ...
        execution = round(
            len(run["replacement_gate_receipts"]) / len(authorized), 6
        )
    else:
    ...
    return {
        ...
        "opportunity_receipt_coverage": round(
            len(run["opportunity_routing_receipts"]) / expected, 6
        ),
...
def _validate_preregistration(preregistration, corpus):
    ...
    if (
        ...
        raise ValueError("opportunity_routing_preregistration_invalid")
...
def _validate_run(run, corpus, preregistration):
    ...
    if (
        ...
        raise ValueError("opportunity_routing_run_invalid")
...
def _checkpoint(
    ...
    counter_valid, failures, opportunity_routing_receipts,
):
    if callback is None:
        return
    commitment = {
        "progress_version": "opportunity_routing_progress_v0_46",
        "source_corpus_hash": corpus["artifact_hash"],
        "source_preregistration_hash": preregistration["artifact_hash"],
        ...
        "opportunity_routing_receipts": dict(
            opportunity_routing_receipts
        ),
