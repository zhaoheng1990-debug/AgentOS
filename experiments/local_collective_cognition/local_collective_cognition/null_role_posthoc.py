...
from .null_role_experiment import RUNTIME_VERSION
from .null_role_holdout import REPLICATION_IDS


POSTHOC_VERSION = "null_role_posthoc_v0_40"


def analyze_null_role_posthoc(*, corpus, run, analysis):
    ...
    for key, composition in sorted(run["composition_receipts"].items()):
        ...
        if rep not in REPLICATION_IDS or arm != "A2_NULL_ROLE_COMPOSED":
            continue
        item = items[case_id]
        ...
        for scored in composition["scored_candidates"]:
            ...
            null_role = scored["provider_candidate_value"]
            value_outcomes[(
                scored["source_id"],
                null_role["research_value_disposition"],
                null_role["relation_truth_state"],
                outcome,
            )] += 1
            if null_role["relation_truth_state"] == "SUPPORTED_NULL":
                null_outcomes[(
                    null_role["null_information_role"],
                    null_role["research_value_disposition"],
                    outcome,
                )] += 1
    ...
    commitment = {
        ...
        "null_role_outcome_distribution": {
