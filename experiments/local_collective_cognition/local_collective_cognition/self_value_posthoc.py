"""Private-outcome calibration and oracle audit for v0.38."""
...
from .self_value_experiment import RUNTIME_VERSION
from .self_value_holdout import REPLICATION_IDS


POSTHOC_VERSION = "self_value_posthoc_v0_38"


def analyze_self_value_posthoc(*, corpus, run, analysis):
    ...
    for key, composition in sorted(run["composition_receipts"].items()):
        ...
        if rep not in REPLICATION_IDS or arm != "A2_SELF_COMPOSED":
            continue
        item = items[case_id]
        ...
        for scored in composition["scored_candidates"]:
            ...
            self_value = scored["provider_self_value"]
            value_outcomes[(
                scored["source_id"],
                self_value["verdict"],
                self_value["relation_evidence"],
                outcome,
            )] += 1
    ...
    commitment = {
        ...
        "self_value_outcome_distribution": {
            f"{source}:{verdict}:{relation}:{outcome}": count
            for (source, verdict, relation, outcome), count in sorted(
