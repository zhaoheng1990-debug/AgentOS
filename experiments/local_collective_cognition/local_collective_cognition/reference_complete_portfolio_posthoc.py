"""Private-outcome audit for portfolio displacement v0.51."""
...
from .reference_complete_portfolio_experiment import RUNTIME_VERSION, SHARED_ARM_ID
from .reference_complete_portfolio_holdout import REPLICATION_IDS


POSTHOC_VERSION = "reference_complete_portfolio_posthoc_v0_51"


def analyze_reference_complete_portfolio_posthoc(*, corpus, run, analysis):
    ...
    for key, gate in sorted(run["replacement_gate_receipts"].items()):
        ...
        cells.append({
            ...
            "opportunity_routing": copy.deepcopy(
                run["reference_complete_portfolio_receipts"][key]
            ),
            "pairwise_displacement_receipt": copy.deepcopy(
                run["pairwise_displacement_receipts"][key]
            ),
            "pairwise_displacement_view_hash": run[
    ...
    commitment = {
        ...
        "proposed_gross_uplift": _safe_metrics(proposed_uplifts),
        "gated_gross_uplift": _safe_metrics(gated_uplifts),
        "pool_oracle_gross_uplift": _safe_metrics(oracle_uplifts),
        "gate_regret": _safe_metrics(regrets),
