"""Private-outcome calibration and oracle audit for v0.39."""
...
from .truth_value_experiment import RUNTIME_VERSION
from .truth_value_holdout import REPLICATION_IDS


POSTHOC_VERSION = "truth_value_posthoc_v0_39"


def analyze_truth_value_posthoc(*, corpus, run, analysis):
    ...
    for key, composition in sorted(run["composition_receipts"].items()):
        ...
        if rep not in REPLICATION_IDS or arm != "A2_TRUTH_COMPOSED":
            continue
        item = items[case_id]
        ...
        for scored in composition["scored_candidates"]:
            ...
            truth_value = scored["provider_candidate_value"]
            value_outcomes[(
                scored["source_id"],
                truth_value["research_value_disposition"],
                truth_value["relation_truth_state"],
                outcome,
            )] += 1
            if truth_value["relation_truth_state"] == "SUPPORTED_NULL":
                null_outcomes[(
                    truth_value["null_discrimination"],
                    truth_value["research_value_disposition"],
                    outcome,
                )] += 1
    ...
    commitment = {
        ...
        "truth_value_outcome_distribution": {
