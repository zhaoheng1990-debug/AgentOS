...
from .review_ready_holdout import (
    REPLICATION_IDS,
    validate_review_ready_holdout,
)
...
from .review_ready_lineage_contract import (
    CONTRACT_VERSION as LINEAGE_CONTRACT_VERSION,
    build_review_ready_delta_view,
    review_ready_delta_schema,
    validate_review_ready_delta,
)
...
RUNTIME_VERSION = "review_ready_experiment_v0_48"
ARM_IDS = ("A1_BASELINE", "A2_COMPACT_DELTA")
SHARED_ARM_ID = "SHARED_STANDARD"


def build_review_ready_preregistration(
    *, corpus, prior_analysis, prior_closure, prior_posthoc
):
    validate_review_ready_holdout(corpus)
    if (
        prior_analysis.get("decision")
        != "REJECT_ADMISSION_LINEAGE_DISPLACEMENT"
        or prior_analysis.get("candidate_state")
        != "LINEAGE_DISPLACEMENT_REJECTED_STOP"
        ...
        or prior_posthoc.get("positive_oracle_cell_count") != 0
        or prior_posthoc.get("classification_distribution", {}) != {}
    ):
        raise ValueError("review_ready_prior_invalid")
    ...
    commitment = {
        ...
        "frozen_hypothesis": (
            "Separating review-ready from admission-ready semantics will "
            "allow supported informative-null opportunities with RETAIN, "
            "ADEQUATE binding, and positive expected Cbit to reach an "
            "independent pairwise review. Full semantic-axis lineage must "
            "remain immutable, and final replacement still requires the "
            "arbiter to prefer the delta over the exact displaced base "
            "candidate. This should restore gray-zone review without "
            "reintroducing the harmful promotion observed in v0.46."
        ),
        ...
        "success_gate": {
            ...
            "minimum_accepted_composition_count": 0,
            "minimum_distinct_acceptance_lanes": 0,
            "minimum_triggered_composition_gross_mean": 0.0,
            "minimum_triggered_composition_gross_median": 0.0,
            "minimum_triggered_composition_gross_win_rate": 0.0,
            ...
            "minimum_pairwise_review_count": 1,
            "minimum_pairwise_review_coverage": 0.9,
            "minimum_fail_closed_or_reviewed_count": 1,
        ...
        "review_ready_adds_provider_calls": True,
...
def run_review_ready_experiment(
    *, corpus, preregistration, adapter, checkpoint_callback=None
    ...
    for key, qualification in sorted(
        ...
        routing = review_ready_receipts[key]
        ...
        delta_view = build_review_ready_delta_view(
            raw_receipt=provisional,
            item=item,
            qualification_receipt=qualification,
        ...
        if envelope.status != "COMPLETED":
            ...
            contract_failures = validate_review_ready_delta(
                delta=delta,
                item=delta_view,
                refs=refs,
            )
    ...
    commitment = {
        ...
        "review_ready_receipts": review_ready_receipts,
...
def analyze_review_ready_experiment(*, corpus, preregistration, run):
    validate_review_ready_holdout(corpus)
    _validate_preregistration(preregistration, corpus)
    ...
    conditions = {
        ...
        "compact_delta_boundary_preserved": (
            ...
            and run["review_ready_adds_provider_calls"] is True
            and run["lineage_warrant_preserved"] is True
            and run[
                "pairwise_displacement_review_required"
            ] is True
            and run["review_ready_is_not_admission_ready"] is True
            and all(
                value["private_truth_used"] is False
    ...
    if passed:
        decision = "PASS_ADMISSION_REVIEW_READY"
        state = "REVIEW_READY_PASSED_READY_POSTHOC"
        ...
        state = "REVIEW_READY_REJECTED_STOP"
    commitment = {
        ...
        "review_ready_gate": "PASS" if passed else "REJECT",
        "decision": decision,
    ...
    *, item, replication_id, experimental_arm_id, route_id,
    ...
    return ProviderCognitiveTask(
        ...
        objective=(
            ...
            "opportunity_bindings. Also echo every required "
        ...
        allowed_evidence=list(refs),
        expected_schema=review_ready_delta_schema(item=item, refs=refs),
        timeout_seconds=min(600, adapter.profile.max_timeout_seconds),
        failure_semantics="preserve_failure",
    )
...
def _runtime_metrics(run, corpus):
    ...
    authorized = {
        key
        for key, value in run["review_ready_receipts"].items()
    ...
    return {
        ...
        "opportunity_receipt_coverage": round(
            len(run["review_ready_receipts"]) / expected, 6
        ),
...
def _validate_preregistration(preregistration, corpus):
    ...
    if (
        ...
        raise ValueError("review_ready_preregistration_invalid")
...
def _validate_run(run, corpus, preregistration):
    ...
    if (
        ...
        raise ValueError("review_ready_run_invalid")
...
def _checkpoint(
    ...
    counter_valid, failures, review_ready_receipts,
    lineage_valid, pairwise_views, pairwise_receipts, pairwise_valid,
):
    if callback is None:
        return
    commitment = {
        "progress_version": "review_ready_progress_v0_48",
        "source_corpus_hash": corpus["artifact_hash"],
        "source_preregistration_hash": preregistration["artifact_hash"],
