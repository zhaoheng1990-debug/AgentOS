"""Anonymous adjudication and typed reference for admission v0.82."""
...
from .admission_v7_external_panel import (
    PANEL_VERSION,
...
ADJUDICATION_VERSION = "admission_v7_staged_context_adjudication_v0_82"
REFERENCE_VERSION = "admission_v7_staged_context_reference_v0_82"
LABEL_FIELDS = (
    "object_relation",
    "evidence_utility",
    "disposition",
...
def build_adjudication(*, packs, panel_manifest, responses):
    ...
    if (
        ...
        raise ValueError("admission_v7_annotation_lanes_invalid")
    ...
    for unit_key in sorted(public_units):
        ...
        if len({hash_payload(value) for value in semantic.values()}) == 1:
            ...
            continue
        adjudication_id = "admission-v7-adjudication-" + hash_payload([
    ...
    pack_value = {
        ...
        "instructions": (
            ...
            "use INDEPENDENT_REASSESSMENT. Retain non-effect context only when "
            "it materially identifies, interprets, scopes, or validates the "
            "exact target. Reject mere topical similarity or a different "
            "outcome. A separately interpretable target effect remains "
            "evidence, including null, uncertainty, or corroboration."
        ),
        "items": disagreements,
...
def validate_adjudication(*, pack, manifest):
    ...
    if (
        ...
        or manifest.get("baseline_outputs_exposed_to_adjudicator") is not False
        or manifest.get("atomic_outputs_exposed_to_adjudicator") is not False
        or manifest.get("candidate_outputs_exposed_to_adjudicator") is not False
    ):
        raise ValueError("admission_v7_adjudication_invalid")
    forbidden = ("gpt-5.6", "gemini-3.1", "openai", "google", "deepseek")
    if any(value in str(pack).casefold() for value in forbidden):
        raise ValueError("admission_v7_adjudication_identity_leak")
...
def validate_adjudication_response(response, *, pack):
    ...
    if (
        ...
        raise ValueError("admission_v7_adjudication_response_binding_invalid")
    items = {
        item["adjudication_id"]: item for item in pack["items"]
    }
    decisions = response.get("decisions")
    if not isinstance(decisions, list) or len(decisions) != len(items):
        raise ValueError("admission_v7_adjudication_response_count_invalid")
    observed = []
    for decision in decisions:
        if (
            ...
            raise ValueError("admission_v7_adjudication_decision_shape_invalid")
        ...
        if (
            ...
            raise ValueError("admission_v7_adjudication_decision_invalid")
        positions = {
        ...
        if basis in positions and _semantic_label(decision) != positions[basis]:
            raise ValueError("admission_v7_adjudication_position_mismatch")
    if len(observed) != len(set(observed)) or set(observed) != set(items):
        raise ValueError("admission_v7_adjudication_ids_invalid")
    ...
    *,
    ...
    if adjudication_pack["items"]:
        if adjudication_response is None:
            raise ValueError("admission_v7_adjudication_response_required")
