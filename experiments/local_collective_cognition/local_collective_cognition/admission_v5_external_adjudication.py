"""Anonymous adjudication and typed reference for v0.80."""
...
ADJUDICATION_VERSION = "admission_v5_atomic_typed_adjudication_v0_80"
REFERENCE_VERSION = "admission_v5_atomic_typed_reference_v0_80"
LABEL_FIELDS = (
    "object_relation",
    "evidence_utility",
    "disposition",
...
def build_adjudication(*, packs, panel_manifest, responses):
    ...
    if (
        ...
        raise ValueError("admission_v5_annotation_lanes_invalid")
    ...
    for unit_key in sorted(public_units):
        ...
        if len({hash_payload(value) for value in semantic.values()}) == 1:
            ...
            continue
        adjudication_id = "admission-v5-adjudication-" + hash_payload([
    ...
    pack_value = {
        ...
        "instructions": (
            ...
            "use INDEPENDENT_REASSESSMENT. A separately interpretable target "
            "value remains effect-bearing even beside other outcomes. A "
            "non-separable composite cannot support one component. Null, "
            "uncertainty, significance, and corroboration may be evidence."
        ),
        "items": disagreements,
...
def validate_adjudication_response(response, *, pack):
    ...
    if (
        ...
        raise ValueError("admission_v5_adjudication_response_binding_invalid")
    items = {
        item["adjudication_id"]: item for item in pack["items"]
    }
    decisions = response.get("decisions")
    if not isinstance(decisions, list) or len(decisions) != len(items):
        raise ValueError("admission_v5_adjudication_response_count_invalid")
    observed = []
    for decision in decisions:
        if (
            ...
            raise ValueError("admission_v5_adjudication_decision_shape_invalid")
        ...
        if (
            ...
            raise ValueError("admission_v5_adjudication_decision_invalid")
        positions = {
        ...
        if basis in positions and _semantic_label(decision) != positions[basis]:
            raise ValueError("admission_v5_adjudication_position_mismatch")
    if len(observed) != len(set(observed)) or set(observed) != set(items):
        raise ValueError("admission_v5_adjudication_ids_invalid")


def build_typed_reference(
    *, panel_manifest, adjudication_pack, adjudication_manifest,
    ...
    if adjudication_pack["items"]:
        if adjudication_response is None:
            raise ValueError("admission_v5_adjudication_response_required")
...
def _validate_hash(value, field):
    commitment = {key: item for key, item in value.items() if key != field}
    if value.get(field) != hash_payload(commitment):
        raise ValueError(f"admission_v5_{field}_invalid")
