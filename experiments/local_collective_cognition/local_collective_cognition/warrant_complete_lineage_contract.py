"""Schema-enforced warrant and semantic lineage binding for v0.49."""
...
CONTRACT_VERSION = "warrant_complete_lineage_contract_v0_49"
BOUND_AXES = (
...
def build_warrant_complete_delta_view(
    *, raw_receipt, item, qualification_receipt, routing_receipt
    ...
    commitment["view_version"] = "warrant_complete_delta_view_v0_49"
    from .provider_telemetry import hash_payload
    return {**commitment, "artifact_hash": hash_payload(commitment)}


def warrant_complete_delta_schema(*, item, refs):
    schema = opportunity_delta_schema(item=item, refs=refs)
    warrant_sets = [
        value["evidence_span_ids"]
        for value in item["opportunity_bindings"]
    ]
    schema["properties"]["evidence_span_ids"]["enum"] = warrant_sets
    lineage_fields = [
...
def validate_warrant_complete_delta(*, delta, item, refs):
    ...
    if binding is None:
        return [*failures, "WARRANT_LINEAGE_BINDING_MISSING"]
    exact = {
    ...
    for field, expected in exact.items():
        if delta.get(field) != expected:
            failures.append(f"WARRANT_LINEAGE_{field.upper()}_MISMATCH")
    if delta.get("evidence_span_ids") != binding["evidence_span_ids"]:
        failures.append("WARRANT_LINEAGE_EVIDENCE_SET_NOT_EXACT")
    for axis in BOUND_AXES:
        if delta.get(axis) != binding[axis]:
            failures.append(
                f"WARRANT_LINEAGE_SEMANTIC_AXIS_DRIFT:{axis}"
            )
    return failures
