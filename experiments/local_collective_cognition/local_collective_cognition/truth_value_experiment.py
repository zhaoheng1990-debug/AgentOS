"""Truth-state/research-value separated composition experiment v0.39."""
...
from .truth_value_holdout import (
    REPLICATION_IDS,
    validate_truth_value_holdout,
)
from .truth_value_candidate_composition import (
    CONTRACT_VERSION,
    SCORE_MAP,
    SOURCE_RANK_BONUS,
    compose_truth_valued_pool,
    truth_valued_exact_three_schema,
    validate_truth_valued_receipt,
)


RUNTIME_VERSION = "truth_value_experiment_v0_39"
ARM_IDS = ("A1_BASELINE", "A2_TRUTH_COMPOSED")


def build_truth_value_preregistration(
    *, corpus, prior_analysis, prior_closure, prior_posthoc
):
    validate_truth_value_holdout(corpus)
    null_rejections = sum(
        count
        for key, count in prior_posthoc.get(
            "self_value_outcome_distribution", {}
        ).items()
        if ":REJECT:" in key and key.endswith(":INFORMATIVE_NULL")
    )
    if (
        prior_analysis.get("decision")
        != "REJECT_SELF_VALUE_COMPOSITION"
        or prior_analysis.get("candidate_state")
        != "SELF_VALUE_REJECTED_STOP"
        or prior_closure.get("candidate_state")
        != "SELF_VALUE_REJECTED_STOP"
        or prior_posthoc.get("formal_decision_unchanged") is not True
        or prior_analysis.get("pooled_contrast_metrics", {}).get(
            "mean", -1
        ) < -0.05
        or null_rejections <= 0
    ):
        raise ValueError("truth_value_prior_invalid")
    ...
    commitment = {
        ...
        "source_truth_value_contract_version": CONTRACT_VERSION,
        "source_score_map_hash": hash_payload(SCORE_MAP),
        "source_rank_bonus": list(SOURCE_RANK_BONUS),
        "frozen_hypothesis": (
            "Separating evidence truth state from research-value "
            "disposition, and explicitly representing whether a supported "
            "null rules out a plausible cause, can preserve informative "
            "negative candidates without adding a third Provider call."
        ),
        "replication_ids": list(REPLICATION_IDS),
        "candidate_count_per_final_receipt": 3,
        "primary_contrast": "A2_TRUTH_COMPOSED_MINUS_A1_BASELINE",
        "success_gate": {
            "minimum_provider_completion_per_required_stage": 0.95,
            "minimum_prompt_identity_coverage": 1.0,
            "minimum_standard_truth_value_contract_coverage": 0.95,
            "minimum_counter_truth_value_contract_coverage": 0.9,
        ...
        "cost_policy": {
            ...
            "standard_truth_value_cost_symmetric_between_arms": True,
...
def run_truth_value_experiment(
    *, corpus, preregistration, adapter, checkpoint_callback=None
        ...
        if envelope.status != "COMPLETED":
            ...
            if contract_failures:
                failures.append({
                    ...
                    "stage": "STANDARD_TRUTH_VALUE_CONTRACT",
    ...
    for key, trigger in sorted(trigger_receipts.items()):
        ...
        provisional = raw_receipts[
            f"{rep}:{arm}:STANDARD_TRUTH_VALUE:{case_id}"
        ]
        task = _truth_value_task(
            item=item, replication_id=rep,
        ...
        calls.append(call)
        raw_key = (
            f"{rep}:{arm}:INDEPENDENT_COUNTER_TRUTH_VALUE:{case_id}"
        )
        if envelope.status != "COMPLETED":
            failures.append(_provider_failure(call))
        else:
            ...
            if contract_failures:
                failures.append({
                    ...
                    "arm_id": arm,
                    "case_id": case_id,
                    "stage": "COUNTER_TRUTH_VALUE_CONTRACT",
                    "failures": contract_failures,
                ...
                failures.append({
                    ...
                    "failures": ["STANDARD_TRUTH_VALUE_INVALID"],
                })
            else:
                ...
                final_raw, composition = compose_truth_valued_pool(
    ...
    commitment = {
        ...
        "standard_truth_value_valid_keys": standard_valid,
        "counter_truth_value_valid_keys": counter_valid,
        "failures": failures,
...
def analyze_truth_value_experiment(*, corpus, preregistration, run):
    validate_truth_value_holdout(corpus)
    _validate_preregistration(preregistration, corpus)
    ...
    for rep in REPLICATION_IDS:
        ...
        deltas = {
            item["case_id"]: round(
                totals[(item["case_id"], "A2_TRUTH_COMPOSED")]["net"]
    ...
    conditions = {
        ...
        "minimum_standard_truth_value_contract_coverage": (
            runtime["standard_truth_value_contract_coverage"]
            >= gate["minimum_standard_truth_value_contract_coverage"]
        ),
        "minimum_counter_truth_value_contract_coverage": (
            runtime["counter_truth_value_contract_coverage"]
            >= gate["minimum_counter_truth_value_contract_coverage"]
        ),
        ...
        "truth_value_boundary_preserved": (
    ...
    if passed:
        decision = "PASS_TRUTH_VALUE_COMPOSITION"
        state = "TRUTH_VALUE_PASSED_READY_EXTERNAL_PANEL"
    elif total_tokens > gate["hard_runaway_total_tokens"]:
        decision = "STOP_RESOURCE_RUNAWAY"
        state = "TRUTH_VALUE_RESOURCE_STOP"
    else:
        decision = "REJECT_TRUTH_VALUE_COMPOSITION"
        state = "TRUTH_VALUE_REJECTED_STOP"
    commitment = {
        ...
        "truth_value_gate": "PASS" if passed else "REJECT",
        "decision": decision,
    ...
    *, item, replication_id, experimental_arm_id, route_id,
    ...
    return ProviderCognitiveTask(
        ...
        objective=(
            ...
            "record relation_truth_state independently from "
            "research_value_disposition. If evidence supports no effect, "
            "record whether that null rules out a plausible cause, "
            "constrains a boundary, or is redundant. A null that rules out "
            "a plausible cause can have positive Cbit and must not be "
        ...
        expected_schema=truth_valued_exact_three_schema(
...
def _scoring_run(run, *, replication_id, provisional=False):
    ...
    case_ids = sorted({
        ...
        and value["stage"] == "STANDARD_TRUTH_VALUE"
    })
    for case_id in case_ids:
        for arm in ARM_IDS:
            base = _find_call(
                run, replication_id=replication_id, arm_id=arm,
                case_id=case_id, stage="STANDARD_TRUTH_VALUE",
            )
            path = [base] if base is not None else []
            if arm == "A2_TRUTH_COMPOSED" and not provisional:
                counter = _find_call(
                    run, replication_id=replication_id, arm_id=arm,
                    case_id=case_id,
                    stage="INDEPENDENT_COUNTER_TRUTH_VALUE",
            ...
            projection = (
                ...
                if arm == "A2_TRUTH_COMPOSED" and provisional
...
def _runtime_metrics(run, corpus):
    ...
    for arm in ARM_IDS:
        values = [
            ...
            if value["stage"] == "STANDARD_TRUTH_VALUE"
    ...
    counter_calls = [
        value for value in run["task_calls"]
        if value["stage"] == "INDEPENDENT_COUNTER_TRUTH_VALUE"
    ]
    if triggered:
        completion["INDEPENDENT_COUNTER_TRUTH_VALUE"] = round(
            sum(value["status"] == "COMPLETED" for value in counter_calls)
            / len(triggered), 6
        )
        counter_contract = round(
            len(run["counter_truth_value_valid_keys"])
            / len(triggered), 6
        )
        execution = round(
        ...
        completion["INDEPENDENT_COUNTER_TRUTH_VALUE"] = 1.0
    ...
    for rep in REPLICATION_IDS:
        for item in corpus["public_surface"]["items"]:
            left = _find_call(
                ...
                case_id=item["case_id"], stage="STANDARD_TRUTH_VALUE",
            )
            right = _find_call(
                run, replication_id=rep, arm_id="A2_TRUTH_COMPOSED",
                case_id=item["case_id"], stage="STANDARD_TRUTH_VALUE",
            )
            identity += bool(
                left and right
    ...
    return {
        ...
        "standard_truth_value_contract_coverage": round(
            len(run["standard_truth_value_valid_keys"])
            / (expected * len(ARM_IDS)), 6
        ),
        "counter_truth_value_contract_coverage": counter_contract,
...
def _composition_metrics(
    *, corpus, run, formal_ledgers, provisional_ledgers
    ...
    for rep in REPLICATION_IDS:
        ...
        for item in corpus["public_surface"]["items"]:
            case = item["case_id"]
            key = f"{rep}:A2_TRUTH_COMPOSED:{case}"
            ...
            gross_uplifts.append(
                after[(case, "A2_TRUTH_COMPOSED")]["gross"]
                - before[(case, "A2_TRUTH_COMPOSED")]["gross"]
            )
            net_uplifts.append(
                after[(case, "A2_TRUTH_COMPOSED")]["net"]
                - before[(case, "A2_TRUTH_COMPOSED")]["net"]
            )
            token_penalties.append(
                after[(case, "A2_TRUTH_COMPOSED")]["token_cost"]
                - before[(case, "A2_TRUTH_COMPOSED")]["token_cost"]
            )
    ...
    return {
        ...
        "truth_value_axis_distribution": dict(axes),
...
def _validate_preregistration(preregistration, corpus):
    ...
    if (
        ...
        raise ValueError("truth_value_preregistration_invalid")
...
def _validate_run(run, corpus, preregistration):
    ...
    if (
        ...
        raise ValueError("truth_value_run_invalid")
...
def _checkpoint(
    ...
    if callback is None:
        return
    commitment = {
        "progress_version": "truth_value_progress_v0_39",
        "source_corpus_hash": corpus["artifact_hash"],
        "source_preregistration_hash": preregistration["artifact_hash"],
        ...
        "standard_truth_value_valid_keys": list(standard_valid),
        "counter_truth_value_valid_keys": list(counter_valid),
