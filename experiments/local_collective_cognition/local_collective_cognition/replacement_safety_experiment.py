"""Post-provider replacement-safety experiment v0.43."""
...
from .replacement_safety_holdout import (
    REPLICATION_IDS,
    validate_replacement_safety_holdout,
)
from .replacement_safety_gate import (
...
def build_replacement_safety_preregistration(
    *, corpus, prior_analysis, prior_closure, prior_posthoc
    ...
    if (
        ...
        or prior_posthoc.get("invalid_delta_contract_count") != 0
    ):
        raise ValueError("replacement_safety_prior_invalid")
    ...
    commitment = {
        ...
        "frozen_hypothesis": (
            "A zero-call replacement gate will eliminate gross harmful "
            "replacement while retaining useful compact deltas. It admits "
            "only replacement of a mechanically quarantined base candidate "
            "or a direct focal relation marked SUPPORTED_EFFECT, ADEQUATE, "
            "and HIGH, while protecting a dropped SUPPORTED_NULL."
        ),
        ...
        "success_gate": {
            ...
            "minimum_mean_gain": -0.25,
            "minimum_median_gain": -0.25,
            "minimum_win_rate": 0.3,
            "minimum_positive_replications": 1,
            "minimum_majority_positive_case_rate": 0.25,
            "maximum_severe_loss_rate": 0.25,
...
def run_replacement_safety_experiment(
    *, corpus, preregistration, adapter, checkpoint_callback=None
    ...
    for rep, arm, canonical in matrix:
        ...
        raw_key = f"{rep}:{arm}:STANDARD_COMPACT_DELTA:{item['case_id']}"
...
def analyze_replacement_safety_experiment(*, corpus, preregistration, run):
    validate_replacement_safety_holdout(corpus)
    _validate_preregistration(preregistration, corpus)
    ...
    if passed:
        decision = "PASS_REPLACEMENT_SAFETY_GATE"
        state = "REPLACEMENT_SAFETY_PASSED_READY_EXTERNAL_PANEL"
    elif total_tokens > gate["hard_runaway_total_tokens"]:
        decision = "STOP_RESOURCE_RUNAWAY"
        state = "COMPACT_DELTA_RESOURCE_STOP"
    else:
        decision = "REJECT_REPLACEMENT_SAFETY_GATE"
        state = "REPLACEMENT_SAFETY_REJECTED_STOP"
    commitment = {
        ...
        "replacement_safety_gate": "PASS" if passed else "REJECT",
        "decision": decision,
    ...
    for rep in REPLICATION_IDS:
        for item in corpus["public_surface"]["items"]:
            left = _find_call(
                ...
                case_id=item["case_id"], stage="STANDARD_COMPACT_DELTA",
            )
            right = _find_call(
                run, replication_id=rep, arm_id="A2_COMPACT_DELTA",
                case_id=item["case_id"], stage="STANDARD_COMPACT_DELTA",
            )
            identity += bool(
                left and right
...
def _validate_preregistration(preregistration, corpus):
    ...
    if (
        ...
        raise ValueError("replacement_safety_preregistration_invalid")
...
def _validate_run(run, corpus, preregistration):
    ...
    if (
        ...
        raise ValueError("replacement_safety_run_invalid")
...
def _checkpoint(
    ...
    if callback is None:
        return
    commitment = {
        "progress_version": "replacement_safety_progress_v0_43",
        "source_corpus_hash": corpus["artifact_hash"],
        "source_preregistration_hash": preregistration["artifact_hash"],
