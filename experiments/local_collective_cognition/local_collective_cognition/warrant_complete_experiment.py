"""Warrant-complete routing experiment v0.49."""
...
from .warrant_complete_holdout import (
    REPLICATION_IDS,
    ...
    *, corpus, prior_analysis, prior_closure, prior_posthoc
    ...
    if (
        ...
        or prior_posthoc.get("classification_distribution", {}) != {
            "BENEFICIAL_ACCEPTED": 2
        }
    ):
        raise ValueError("warrant_complete_prior_invalid")
    ...
    commitment = {
        ...
        "source_replacement_gate_version": LINEAGE_GATE_VERSION,
        "source_score_map_hash": hash_payload(SCORE_MAP),
        "source_rank_bonus": list(SOURCE_RANK_BONUS),
        "frozen_hypothesis": (
            "On a fresh holdout, schema-enforced exact preservation of the "
            "source warrant set should eliminate the incomplete-lineage "
            "failure observed in v0.48 while preserving its positive gross "
            "Cbit signal. Review-ready remains distinct from admission-ready, "
            "and final replacement still requires independent pairwise "
            "preference over the exact displaced base candidate."
        ),
        ...
        "success_gate": {
            ...
            "minimum_pre_call_skip_count": 1,
            "minimum_authorized_delta_call_count": 1,
            "maximum_authorized_delta_call_rate": 0.75,
        ...
        "warrant_complete_adds_provider_calls": True,
        ...
        "warrant_complete_is_not_admission_ready": True,
...
def run_warrant_complete_experiment(
    *, corpus, preregistration, adapter, checkpoint_callback=None
    ...
    for key, qualification in sorted(
        ...
        routing = warrant_complete_receipts[key]
        ...
        if envelope.status != "COMPLETED":
            ...
            if contract_failures:
                ...
                pairwise_view = build_pairwise_displacement_view(
                    item=item,
                    provisional_receipt=provisional,
                ...
                review_failures = validate_pairwise_displacement(
                    receipt=pairwise,
                    item=pairwise_view,
                    refs=refs,
                )
    ...
    commitment = {
        ...
        "warrant_complete_receipts": warrant_complete_receipts,
...
def analyze_warrant_complete_experiment(*, corpus, preregistration, run):
    validate_warrant_complete_holdout(corpus)
    _validate_preregistration(preregistration, corpus)
    ...
    conditions = {
        ...
        "minimum_pre_call_skip_count": (
            runtime["pre_call_skip_count"]
            >= gate["minimum_pre_call_skip_count"]
        ),
        "minimum_authorized_delta_call_count": (
            runtime["authorized_delta_call_count"]
            >= gate["minimum_authorized_delta_call_count"]
        ),
        "maximum_authorized_delta_call_rate": (
            runtime["authorized_delta_call_rate"]
            <= gate["maximum_authorized_delta_call_rate"]
        ),
        ...
        "compact_delta_boundary_preserved": (
            ...
            and run["warrant_complete_adds_provider_calls"] is True
            ...
            and run["warrant_complete_is_not_admission_ready"] is True
            and all(
                value["private_truth_used"] is False
    ...
    if passed:
        decision = "PASS_WARRANT_COMPLETE_REPLICATION"
        state = "WARRANT_COMPLETE_REPLICATED_READY_POSTHOC"
    elif total_tokens > gate["hard_runaway_total_tokens"]:
        decision = "STOP_RESOURCE_RUNAWAY"
        state = "COMPACT_DELTA_RESOURCE_STOP"
    else:
        decision = "REJECT_WARRANT_COMPLETE_REPLICATION"
        state = "WARRANT_COMPLETE_REJECTED_STOP"
    commitment = {
        ...
        "warrant_complete_gate": "PASS" if passed else "REJECT",
        "decision": decision,
...
def _pairwise_task(
    *, item, replication_id, experimental_arm_id, refs, adapter
    ...
    return ProviderCognitiveTask(
        ...
        task_kind=TASK_KIND,
        objective=(
            "Act as an independent displacement arbiter. Compare the "
            "delta candidate only with the exact base candidate it would "
            "replace. Judge information value, evidence strength, "
            "constraint binding, falsifiability, and the cost of losing "
            "the base relation. Verify that the delta preserves the bound "
            "opportunity type and warrant. Choose REPLACE only when the "
            "delta is materially stronger than the base candidate and the "
            "lineage is consistent; uncertainty, a tie, or a merely "
            "interesting alternative must KEEP_BASE or be UNRESOLVED. "
        ...
        allowed_evidence=list(refs),
        expected_schema=pairwise_displacement_schema(
            item=item, refs=refs
        ),
        timeout_seconds=min(600, adapter.profile.max_timeout_seconds),
        failure_semantics="preserve_failure",
...
def _runtime_metrics(run, corpus):
    ...
    authorized = {
        key
        for key, value in run["warrant_complete_receipts"].items()
    ...
    return {
        ...
        "opportunity_receipt_coverage": round(
            len(run["warrant_complete_receipts"]) / expected, 6
        ),
...
def _validate_preregistration(preregistration, corpus):
    ...
    if (
        ...
        raise ValueError("warrant_complete_preregistration_invalid")
...
def _validate_run(run, corpus, preregistration):
    ...
    if (
        ...
        raise ValueError("warrant_complete_run_invalid")
...
def _checkpoint(
    ...
    counter_valid, failures, warrant_complete_receipts,
    lineage_valid, pairwise_views, pairwise_receipts, pairwise_valid,
):
    if callback is None:
        return
    commitment = {
        "progress_version": "warrant_complete_progress_v0_49",
        "source_corpus_hash": corpus["artifact_hash"],
        "source_preregistration_hash": preregistration["artifact_hash"],
        ...
        "warrant_complete_receipts": dict(
            warrant_complete_receipts
        ),
