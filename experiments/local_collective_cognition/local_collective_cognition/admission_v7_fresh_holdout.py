"""Fifth unused-source holdout for staged context utility v0.82."""
...
from .admission_v6_fresh_holdout import (
    SELECTED_PROMPT_IDS as V081_PROMPT_IDS,
    prior_surface_ids as v081_prior_surface_ids,
)
...
def prior_surface_ids():
    return v081_prior_surface_ids() | set(V081_PROMPT_IDS)
...
def build_fresh_holdout(paths, *, source_manifest):
    panel = build_selected_panel(
        ...
        benchmark_id="evidence-inference-admission-v7-fresh-holdout",
        split="fifth_unused_test_selection_v0_82",
    ...
    value.update({
        "bridge_version": "admission_v7_fresh_holdout_v0_82",
        "selection_version": "sha256_fifth_unused_test_objects_v0_82",
...
def validate_fresh_holdout(panel):
    ...
    if (
        ...
        raise ValueError("admission_v7_fresh_holdout_invalid")


def build_preregistration(panel):
    validate_fresh_holdout(panel)
    value = {
        "preregistration_version": "admission_v7_fresh_holdout_v0_82",
...
def validate_preregistration(value, *, panel):
    ...
    if (
        ...
        or gate.get("atomic_evidence_partition_invariance_required")
        is not True
        or gate.get("candidate_context_and_reject_both_required")
        is not True
        or gate.get("grounded_context_policy_required") is not True
        or gate.get("effect_rejudgment_allowed") is not False
    ):
        raise ValueError("admission_v7_preregistration_invalid")
