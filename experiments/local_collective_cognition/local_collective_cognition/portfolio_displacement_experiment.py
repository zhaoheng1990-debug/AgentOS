...
from .portfolio_displacement_holdout import (
    REPLICATION_IDS,
    validate_portfolio_displacement_holdout,
)
...
from .portfolio_displacement_contract import (
    CONTRACT_VERSION as PORTFOLIO_CONTRACT_VERSION,
    GATE_VERSION as PORTFOLIO_GATE_VERSION,
    apply_portfolio_displacement_gate,
    build_portfolio_displacement_view,
    portfolio_displacement_schema,
    validate_portfolio_displacement,
)


RUNTIME_VERSION = "portfolio_displacement_experiment_v0_50"
ARM_IDS = ("A1_BASELINE", "A2_COMPACT_DELTA")
SHARED_ARM_ID = "SHARED_STANDARD"


def build_portfolio_displacement_preregistration(
    *, corpus, prior_analysis, prior_closure, prior_posthoc
):
    validate_portfolio_displacement_holdout(corpus)
    if (
        prior_analysis.get("decision")
        != "REJECT_WARRANT_COMPLETE_REPLICATION"
        or prior_analysis.get("candidate_state")
        != "WARRANT_COMPLETE_REJECTED_STOP"
        ...
        or prior_posthoc.get("positive_oracle_cell_count") != 6
        or prior_posthoc.get("classification_distribution", {}) != {
            "BENEFICIAL_ACCEPTED": 1,
            "BENEFICIAL_REJECTED": 5,
            "TIE_REJECTED": 2,
        }
    ):
        raise ValueError("portfolio_displacement_prior_invalid")
    ...
    commitment = {
        ...
        "frozen_hypothesis": (
            "On a fresh holdout, explicitly defining displacement as a "
            "non-destructive active-portfolio choice should prevent the "
            "arbiter from charging permanent deletion cost to a base "
            "candidate that remains preserved. Comparing base and delta by "
            "marginal Cbit relative to two fixed active companions should "
            "recover more beneficial informative-null proposals without "
            "increasing harmful acceptance."
        ),
        ...
        "portfolio_displacement_adds_provider_calls": True,
        ...
        "portfolio_displacement_is_not_admission_ready": True,
...
def run_portfolio_displacement_experiment(
    *, corpus, preregistration, adapter, checkpoint_callback=None
    ...
    portfolio_displacement_receipts = {}
    ...
    for key, qualification in sorted(
        ...
        portfolio_displacement_receipts[key] = (
            derive_review_ready_receipt(
                key=key,
    ...
    for key, qualification in sorted(
        ...
        routing = portfolio_displacement_receipts[key]
        ...
        if envelope.status != "COMPLETED":
            ...
            if contract_failures:
                ...
                pairwise_view = build_portfolio_displacement_view(
                    item=item,
                    provisional_receipt=provisional,
                ...
                review_failures = validate_portfolio_displacement(
                    receipt=pairwise,
                    item=pairwise_view,
                    refs=refs,
                )
    ...
    commitment = {
        ...
        "portfolio_displacement_receipts": portfolio_displacement_receipts,
...
def analyze_portfolio_displacement_experiment(*, corpus, preregistration, run):
    validate_portfolio_displacement_holdout(corpus)
    _validate_preregistration(preregistration, corpus)
    ...
    conditions = {
        ...
        "compact_delta_boundary_preserved": (
            ...
            and run["portfolio_displacement_adds_provider_calls"] is True
            ...
            and run["portfolio_displacement_is_not_admission_ready"] is True
            and all(
                value["private_truth_used"] is False
    ...
    if passed:
        decision = "PASS_PORTFOLIO_DISPLACEMENT_REPLICATION"
        state = "PORTFOLIO_DISPLACEMENT_REPLICATED_READY_POSTHOC"
    elif total_tokens > gate["hard_runaway_total_tokens"]:
        decision = "STOP_RESOURCE_RUNAWAY"
        state = "COMPACT_DELTA_RESOURCE_STOP"
    else:
        decision = "REJECT_PORTFOLIO_DISPLACEMENT_REPLICATION"
        state = "PORTFOLIO_DISPLACEMENT_REJECTED_STOP"
    commitment = {
        ...
        "portfolio_displacement_gate": "PASS" if passed else "REJECT",
        "decision": decision,
...
def _pairwise_task(
    *, item, replication_id, experimental_arm_id, refs, adapter
    ...
    return ProviderCognitiveTask(
        ...
        allowed_evidence=list(refs),
        expected_schema=portfolio_displacement_schema(
            item=item, refs=refs
        ),
        timeout_seconds=min(600, adapter.profile.max_timeout_seconds),
        failure_semantics="preserve_failure",
...
def _runtime_metrics(run, corpus):
    ...
    authorized = {
        key
        for key, value in run["portfolio_displacement_receipts"].items()
    ...
    return {
        ...
        "opportunity_receipt_coverage": round(
            len(run["portfolio_displacement_receipts"]) / expected, 6
        ),
...
def _validate_preregistration(preregistration, corpus):
    ...
    if (
        ...
        raise ValueError("portfolio_displacement_preregistration_invalid")
...
def _validate_run(run, corpus, preregistration):
    ...
    if (
        ...
        raise ValueError("portfolio_displacement_run_invalid")
...
def _checkpoint(
    ...
    counter_valid, failures, portfolio_displacement_receipts,
    lineage_valid, pairwise_views, pairwise_receipts, pairwise_valid,
):
    if callback is None:
        return
    commitment = {
        "progress_version": "portfolio_displacement_progress_v0_50",
        "source_corpus_hash": corpus["artifact_hash"],
        "source_preregistration_hash": preregistration["artifact_hash"],
        ...
        "portfolio_displacement_receipts": dict(
            portfolio_displacement_receipts
        ),
