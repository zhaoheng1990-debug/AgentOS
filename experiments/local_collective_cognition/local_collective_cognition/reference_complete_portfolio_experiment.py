...
from .reference_complete_portfolio_holdout import (
    REPLICATION_IDS,
    validate_reference_complete_portfolio_holdout,
)
...
from .admission_opportunity_contract import (
    ...
    opportunity_enriched_exact_three_schema,
    validate_opportunity_enriched_receipt,
)
from .review_ready_routing import (
    ROUTING_VERSION as OPPORTUNITY_ROUTING_VERSION,
    derive_review_ready_receipt,
)
from .warrant_complete_lineage_contract import (
    CONTRACT_VERSION as LINEAGE_CONTRACT_VERSION,
...
from .lineage_displacement_contract import (
    derive_current_stage_witnesses,
)
from .source_qualified_portfolio_contract import (
    CONTRACT_VERSION as PORTFOLIO_CONTRACT_VERSION,
    GATE_VERSION as PORTFOLIO_GATE_VERSION,
    apply_source_qualified_portfolio_gate,
    build_source_qualified_portfolio_view,
    source_qualified_portfolio_schema,
    validate_source_qualified_portfolio,
)
...
RUNTIME_VERSION = "reference_complete_portfolio_experiment_v0_51"
ARM_IDS = ("A1_BASELINE", "A2_COMPACT_DELTA")
SHARED_ARM_ID = "SHARED_STANDARD"


def build_reference_complete_portfolio_preregistration(
    *,
    corpus,
    reference_audit,
    prior_analysis,
    ...
    validate_reference_complete_portfolio_holdout(corpus)
    validate_reference_completeness_audit(
    ...
    if (
        ...
        or reference_audit.get("reference_authority")
        != "INTERNAL_PROVIDER_AUDITED_SYNTHETIC_ONLY"
    ):
        raise ValueError("reference_complete_portfolio_audit_invalid")
    if (
        prior_analysis.get("decision")
        != "REJECT_PORTFOLIO_DISPLACEMENT_REPLICATION"
        or prior_analysis.get("candidate_state")
        != "PORTFOLIO_DISPLACEMENT_REJECTED_STOP"
        ...
        or prior_posthoc.get("positive_oracle_cell_count") != 2
        or prior_posthoc.get("classification_distribution", {}) != {
            "BENEFICIAL_ACCEPTED": 1,
            "BENEFICIAL_REJECTED": 1,
            "HARMFUL_ACCEPTED": 1,
            "TIE_REJECTED": 1,
        }
    ):
        raise ValueError("reference_complete_portfolio_prior_invalid")
    ...
    commitment = {
        ...
        "source_opportunity_contract_version": (
            OPPORTUNITY_CONTRACT_VERSION
        ),
        "source_opportunity_routing_version": (
            OPPORTUNITY_ROUTING_VERSION
        ),
        "source_lineage_contract_version": LINEAGE_CONTRACT_VERSION,
        "source_portfolio_contract_version": (
            PORTFOLIO_CONTRACT_VERSION
        ),
        "source_composition_contract_version": (
            COMPOSITION_CONTRACT_VERSION
        ),
        "source_replacement_gate_version": PORTFOLIO_GATE_VERSION,
        "source_score_map_hash": hash_payload(SCORE_MAP),
        "source_rank_bonus": list(SOURCE_RANK_BONUS),
        "frozen_hypothesis": (
            "On a fresh relation-complete holdout, source-qualified pool "
            "identities plus a frozen dual-role provider audit should remove "
            "the candidate-identity and incomplete-reference ambiguities "
            "observed in v0.50. The non-destructive arbiter can then be "
            "evaluated on marginal Cbit without treating ambiguous labels "
            "as stable evidence of harmful acceptance."
        ),
        ...
        "success_gate": {
            ...
            "minimum_mean_gain": -10.0,
            "minimum_median_gain": -10.0,
            "minimum_win_rate": 0.0,
            "minimum_positive_replications": 0,
            "minimum_majority_positive_case_rate": 0.0,
            "maximum_severe_loss_rate": 1.0,
            "minimum_relation_jaccard_relative_to_baseline": -0.1,
            "minimum_accepted_composition_count": 2,
            "minimum_distinct_acceptance_lanes": 1,
            ...
            "minimum_opportunity_receipt_coverage": 0.95,
            "minimum_authorized_delta_call_count": 1,
            "minimum_opportunity_constrained_delta_coverage": 0.9,
            "minimum_lineage_contract_coverage": 1.0,
            "minimum_pairwise_review_count": 2,
            "minimum_pairwise_review_coverage": 1.0,
            "minimum_fail_closed_or_reviewed_count": 2,
        ...
        "cost_policy": {
            ...
            "cognitive_quality_primary_cost_secondary": True,
            "soft_token_ceiling_is_observation_only": True,
            "soft_expected_total_tokens": math.ceil(prior_total * 1.2),
            "hard_runaway_total_tokens": math.ceil(prior_total * 1.4),
        },
        ...
        "pairwise_displacement_adds_provider_calls": True,
        ...
        "admission_opportunity_fields_tuned_after_freeze": False,
        "reference_complete_portfolio_adds_provider_calls": True,
        "source_warrant_exact_set_enforced_in_schema": True,
        "delta_relation_must_belong_to_opportunity_set": True,
        "semantic_lane_strengthening_without_new_evidence_allowed": False,
        "pairwise_replacement_review_required": True,
        "source_qualified_pool_ids_required": True,
        "local_candidate_ids_forbidden_in_portfolio_receipt": True,
        "dual_role_reference_audit_required": True,
        "reference_audit_not_external_gold": True,
        "non_destructive_portfolio_review_required": True,
        "base_and_delta_candidate_retention_required": True,
        "marginal_cbit_preference_required": True,
        "reference_complete_portfolio_is_not_admission_ready": True,
...
def run_reference_complete_portfolio_experiment(
    *,
    corpus,
    preregistration,
    adapter,
    ...
    (holdout_validator or validate_reference_complete_portfolio_holdout)(
        corpus
    )
    ...
    reference_complete_portfolio_receipts = {}
    delta_views, materialization_receipts = {}, {}
    candidate_pools, proposed_composition_receipts = {}, {}
    pairwise_views, pairwise_receipts = {}, {}
    pairwise_valid, lineage_valid = [], []
    ...
    for rep, canonical in matrix:
        ...
        task = _standard_task(
            ...
            stage="STANDARD_OPPORTUNITY", refs=refs, adapter=adapter,
        )
        envelope = ProviderTaskRouter([adapter]).route(task)
        call = _call(
            replication_id=rep, arm_id=SHARED_ARM_ID,
            case_id=item["case_id"],
            stage="STANDARD_OPPORTUNITY", route_id="A1_ONTOLOGY",
        ...
        raw_key = (
            f"{rep}:{SHARED_ARM_ID}:STANDARD_OPPORTUNITY:"
            f"{item['case_id']}"
        )
        if envelope.status != "COMPLETED":
            ...
            contract_failures = validate_opportunity_enriched_receipt(
            ...
            if contract_failures:
                failures.append({
                    ...
                    "case_id": item["case_id"],
                    "stage": "STANDARD_OPPORTUNITY_CONTRACT",
            ...
            qualification_receipts[delta_key] = (
                (
                    qualification_deriver
                    or derive_selective_qualification
                )(
                    raw_receipt=raw,
                    item=item,
        ...
        _checkpoint(
            ...
            reference_complete_portfolio_receipts,
            lineage_valid, pairwise_views, pairwise_receipts,
            pairwise_valid,
        )
    items = {
        value["case_id"]: value
    ...
    for key, qualification in sorted(
        ...
        reference_complete_portfolio_receipts[key] = (
            derive_review_ready_receipt(
                key=key,
                raw_receipt=provisional,
                item=item,
                qualification_receipt=qualification,
                all_standard_receipts=raw_receipts,
            )
        )
    for key, qualification in sorted(
        qualification_receipts.items()
    ):
        routing = reference_complete_portfolio_receipts[key]
        if not routing["delta_call_authorized"]:
            continue
        ...
        delta_view = build_warrant_complete_delta_view(
            raw_receipt=provisional,
            item=item,
            qualification_receipt=qualification,
        ...
        if envelope.status != "COMPLETED":
            ...
            contract_failures = validate_warrant_complete_delta(
                delta=delta,
                item=delta_view,
                refs=refs,
            )
            if contract_failures:
                failures.append({
                    ...
                    "arm_id": arm,
                    "case_id": case_id,
                    "stage": "LINEAGE_BOUND_DELTA_CONTRACT",
                    "failures": contract_failures,
                ...
                failures.append({
                    ...
                    "failures": ["STANDARD_OPPORTUNITY_INVALID"],
                })
            else:
                ...
                pairwise_view = build_source_qualified_portfolio_view(
                    item=item,
                    provisional_receipt=provisional,
                ...
                if pairwise_view is None:
                    final_projections[key] = (
                        provisional_projections[key]
                    )
                    continue
                pairwise_views[key] = pairwise_view
                review_task = _pairwise_task(
                    item=pairwise_view,
                    replication_id=rep,
                    experimental_arm_id=arm,
                    refs=refs,
                ...
                review_envelope = ProviderTaskRouter([adapter]).route(
                    review_task
                )
                review_call = _call(
                    replication_id=rep,
                    arm_id=arm,
                    case_id=case_id,
                    stage="PAIRWISE_DISPLACEMENT_REVIEW",
                    route_id="PAIRWISE_ARBITER",
                    task=review_task,
                    envelope=review_envelope,
                )
                calls.append(review_call)
                if review_envelope.status != "COMPLETED":
                    failures.append(_provider_failure(review_call))
                    final_projections[key] = (
                        provisional_projections[key]
                    )
                    continue
                pairwise = review_envelope.normalized_result
                raw_receipts[
                    f"{rep}:{arm}:PAIRWISE_DISPLACEMENT_REVIEW:{case_id}"
                ] = pairwise
                review_failures = validate_source_qualified_portfolio(
                    receipt=pairwise,
                    item=pairwise_view,
                    refs=refs,
                )
                if review_failures:
                    failures.append({
                        ...
                        "arm_id": arm,
                        "case_id": case_id,
                        "stage": "PAIRWISE_DISPLACEMENT_CONTRACT",
                        "failures": review_failures,
                        "raw_receipt_hash": hash_payload(pairwise),
                    })
                    final_projections[key] = (
                        provisional_projections[key]
                    )
                    continue
                pairwise_receipts[key] = pairwise
                pairwise_valid.append(key)
                ...
                final_raw, accepted_composition, gate_receipt = (
                    apply_source_qualified_portfolio_gate(
                        ...
                        proposed_receipt=proposed_raw,
                        composition_receipt=proposed_composition,
                        portfolio_receipt=pairwise,
                        portfolio_view=pairwise_view,
                        lineage_delta=delta,
    ...
    commitment = {
        ...
        "source_reference_audit_hash": preregistration[
            "source_reference_audit_hash"
        ],
        "source_reference_audit_version": REFERENCE_AUDIT_VERSION,
        "reference_audit_claim_ceiling": preregistration[
            "reference_audit_claim_ceiling"
        ],
        "source_escalation_policy_version": ESCALATION_POLICY_VERSION,
        "source_qualification_policy_version": (
        ...
        "reference_complete_portfolio_receipts": reference_complete_portfolio_receipts,
        "delta_views": delta_views,
        "materialization_receipts": materialization_receipts,
        "lineage_valid_keys": lineage_valid,
        "pairwise_displacement_views": pairwise_views,
        "pairwise_displacement_receipts": pairwise_receipts,
        "pairwise_displacement_valid_keys": pairwise_valid,
        ...
        "opportunity_enriched_standard_contract_used": True,
        "delta_relation_constrained_to_opportunity_set": True,
        "reference_complete_portfolio_adds_provider_calls": True,
        "lineage_warrant_preserved": True,
        "source_warrant_exact_set_enforced_in_schema": True,
        "pairwise_displacement_review_required": True,
        "source_qualified_pool_ids_required": True,
        "local_candidate_ids_forbidden_in_portfolio_receipt": True,
        "dual_role_reference_audit_required": True,
        "reference_audit_not_external_gold": True,
        ...
        "base_and_delta_candidate_retention_required": True,
        "marginal_cbit_preference_required": True,
        "permanent_candidate_deletion_allowed": False,
        "reference_complete_portfolio_is_not_admission_ready": True,
        "runtime_materialization_rewrites_semantics": False,
        "replacement_gate_adds_provider_calls": True,
...
def analyze_reference_complete_portfolio_experiment(
    *, corpus, preregistration, run, holdout_validator=None
    ...
    conditions = {
        ...
        "minimum_opportunity_receipt_coverage": (
            runtime["opportunity_receipt_coverage"]
            >= gate["minimum_opportunity_receipt_coverage"]
        ),
        "minimum_authorized_delta_call_count": (
            runtime["authorized_delta_call_count"]
            >= gate["minimum_authorized_delta_call_count"]
        ),
        "minimum_opportunity_constrained_delta_coverage": (
            runtime["opportunity_constrained_delta_coverage"]
            >= gate["minimum_opportunity_constrained_delta_coverage"]
        ),
        "minimum_lineage_contract_coverage": (
            runtime["lineage_contract_coverage"]
            >= gate["minimum_lineage_contract_coverage"]
        ),
        "minimum_pairwise_review_count": (
            runtime["pairwise_review_count"]
            >= gate["minimum_pairwise_review_count"]
        ),
        "minimum_pairwise_review_coverage": (
            runtime["pairwise_review_coverage"]
            >= gate["minimum_pairwise_review_coverage"]
        ),
        "minimum_fail_closed_or_reviewed_count": (
            runtime["fail_closed_or_reviewed_count"]
            >= gate["minimum_fail_closed_or_reviewed_count"]
        ),
        ...
        "minimum_triggered_composition_gross_mean": (
            ...
            ) if gross is not None else replacement["accepted_count"] == 0
        ),
        ...
        "replacement_gate_boundary_preserved": (
            run["replacement_gate_adds_provider_calls"] is True
        ...
        "compact_delta_boundary_preserved": (
            ...
            and run[
                "opportunity_enriched_standard_contract_used"
            ] is True
            and run[
                "delta_relation_constrained_to_opportunity_set"
            ] is True
            and run["reference_complete_portfolio_adds_provider_calls"] is True
            and run["lineage_warrant_preserved"] is True
            and run[
                "source_warrant_exact_set_enforced_in_schema"
            ] is True
            and run[
                "pairwise_displacement_review_required"
            ] is True
            and run["source_qualified_pool_ids_required"] is True
            and run[
                "local_candidate_ids_forbidden_in_portfolio_receipt"
            ] is True
            and run["dual_role_reference_audit_required"] is True
            and run["reference_audit_not_external_gold"] is True
            and run["source_reference_audit_hash"]
            == preregistration["source_reference_audit_hash"]
            and run["source_reference_audit_version"]
            == REFERENCE_AUDIT_VERSION
            and run["reference_audit_claim_ceiling"]
            == "INTERNAL_PROVIDER_AUDITED_SYNTHETIC_ONLY"
            and run[
                "non_destructive_portfolio_review_required"
            ] is True
            and run[
                "base_and_delta_candidate_retention_required"
            ] is True
            and run["marginal_cbit_preference_required"] is True
            and run["permanent_candidate_deletion_allowed"] is False
            and run["reference_complete_portfolio_is_not_admission_ready"] is True
            and all(
                value["private_truth_used"] is False
                and value["provider_call_added_by_routing"] is False
                for value in run[
                    "reference_complete_portfolio_receipts"
                ].values()
            )
    ...
    if passed:
        decision = "PASS_REFERENCE_COMPLETE_PORTFOLIO_REPLICATION"
        state = "REFERENCE_COMPLETE_PORTFOLIO_REPLICATED_READY_POSTHOC"
        ...
        state = "REFERENCE_COMPLETE_PORTFOLIO_REJECTED_STOP"
    commitment = {
        ...
        "cost_observations": {
            "soft_ceiling_tokens": gate["maximum_physical_total_tokens"],
            "soft_ceiling_met": (
                total_tokens <= gate["maximum_physical_total_tokens"]
            ),
            "hard_runaway_tokens": gate["hard_runaway_total_tokens"],
            "mean_added_token_penalty_cbit": composition[
                "mean_added_token_penalty_cbit"
            ],
            "mean_delta_token_penalty_cbit": composition[
                "mean_delta_token_penalty_cbit"
            ],
            "affects_cognitive_quality_gate": False,
        },
        "conditions": conditions,
        "reference_complete_portfolio_gate": "PASS" if passed else "REJECT",
        "decision": decision,
    ...
    *, item, replication_id, experimental_arm_id, route_id,
    ...
    return ProviderCognitiveTask(
        ...
        objective=(
            ...
            "Also return zero to two admission_opportunities outside the "
            "three candidates. Each opportunity is only a routing proposal "
            "for an independent second role, not a final candidate. Include "
            "one only when the evidence supports a focal-object relation "
            "that is admission-ready as an effect, informative null, or "
            "quarantine alternative; otherwise return an empty list. "
        ...
        expected_schema=opportunity_enriched_exact_three_schema(
    ...
    *, item, replication_id, experimental_arm_id, route_id,
    ...
    return ProviderCognitiveTask(
        ...
        objective=(
            ...
            "source_object_id and target_object_id pair must exactly match "
            "one pair in allowed_relations and must not be listed in "
            "excluded_relations. Use only the supplied evidence subset. "
            "Echo the matching opportunity hash, opportunity type, exact "
            "source evidence spans, and required admission lane from "
            "opportunity_bindings. evidence_span_ids must equal the source "
            "opportunity evidence set exactly; omission and unbound additions "
            "are both invalid. Also echo every required "
            "source_opportunity semantic axis. The delta's truth state, "
            "null role, constraint binding, expected Cbit, and research "
            "value disposition must exactly equal those source axes. "
            "These lineage fields are immutable. "
            "Do not strengthen a QUARANTINE_ALTERNATIVE into a supported "
            "effect, supported null, or PRIORITIZE disposition. "
            "Record current "
        ...
        inputs={
            ...
            "allowed_relations": item["allowed_relations"],
            "opportunity_bindings": item["opportunity_bindings"],
        ...
        allowed_evidence=list(refs),
        expected_schema=warrant_complete_delta_schema(item=item, refs=refs),
        timeout_seconds=min(600, adapter.profile.max_timeout_seconds),
        failure_semantics="preserve_failure",
    )
...
def _pairwise_task(
    *, item, replication_id, experimental_arm_id, refs, adapter
    ...
    return ProviderCognitiveTask(
        ...
        task_kind=TASK_KIND,
        objective=(
            "Act as an independent active-portfolio arbiter. The portfolio "
            "has capacity three and two companion candidates are fixed. "
            "Compare only which of the base candidate or delta should occupy "
            "the third active slot now. This is non-destructive: both "
            "candidates remain preserved and can be activated later, so "
            "permanent deletion cost is zero. Judge marginal information "
            "gain relative to the two fixed companions, evidence strength, "
            "hypothesis-space reduction, redundancy, constraint binding, "
            "and falsifiability. A supported informative null may have "
            "higher marginal Cbit than an unverified positive hypothesis; "
            "positive effects have no categorical priority. Verify the "
            "bound opportunity type, warrant, and lineage. Choose "
            "ACTIVATE_DELTA only when the delta has higher marginal Cbit; "
            "a tie or unresolved comparison keeps the base active. "
            "All portfolio identities are source-qualified pool IDs. Echo "
            "the exact BASE:C* and COUNTER:C* IDs from the view; never use "
            "or compare a local C* candidate_id as a portfolio identity. "
            "The Runtime score map and private outcomes are unavailable."
        ),
        inputs={
            "stage": stage,
            "replication_id": replication_id,
        ...
        allowed_evidence=list(refs),
        expected_schema=source_qualified_portfolio_schema(
            item=item, refs=refs
        ),
        timeout_seconds=min(600, adapter.profile.max_timeout_seconds),
        failure_semantics="preserve_failure",
...
def _scoring_run(run, *, replication_id, provisional=False):
    ...
    case_ids = sorted({
        ...
        and value["stage"] == "STANDARD_OPPORTUNITY"
    })
    for case_id in case_ids:
        for arm in ARM_IDS:
            base = _find_call(
                ...
                arm_id=SHARED_ARM_ID,
                case_id=case_id, stage="STANDARD_OPPORTUNITY",
            )
            ...
            if arm == "A2_COMPACT_DELTA" and not provisional:
                ...
                for stage in (
                    "REJECTION_CHALLENGE",
                    "REJECTION_COORDINATION",
                    "BLIND_EVIDENCE_AUDIT",
                    "BLIND_EVIDENCE_SKEPTIC",
                ):
                    selective_call = _find_call(
                        run,
                        replication_id=replication_id,
                        arm_id=arm,
                        case_id=case_id,
                    ...
                    if selective_call is not None:
                        path.append(selective_call)
            calls.append({
                "arm_id": arm,
                "case_id": case_id,
                "invocation_receipt": {
...
def _runtime_metrics(run, corpus):
    ...
    shared_values = [
        ...
        if value["stage"] == "STANDARD_OPPORTUNITY"
        and value["arm_id"] == SHARED_ARM_ID
    ]
    completion = {
    ...
    authorized = {
        key
        for key, value in run["reference_complete_portfolio_receipts"].items()
        if value["delta_call_authorized"]
    }
    counter_calls = [
        value for value in run["task_calls"]
    ...
    pairwise_calls = [
        value for value in run["task_calls"]
        if value["stage"] == "PAIRWISE_DISPLACEMENT_REVIEW"
    ]
    pairwise_expected = len(run["pairwise_displacement_views"])
    if authorized:
        ...
        execution = round(
            len(run["replacement_gate_receipts"])
            / len(run["pairwise_displacement_valid_keys"])
            if run["pairwise_displacement_valid_keys"] else 1.0,
            6,
        )
    else:
    ...
    if pairwise_expected:
        completion["PAIRWISE_DISPLACEMENT_REVIEW"] = round(
            sum(
                value["status"] == "COMPLETED"
                for value in pairwise_calls
            ) / pairwise_expected,
            6,
        )
    else:
        completion["PAIRWISE_DISPLACEMENT_REVIEW"] = 1.0
    identity = 0
    for rep in REPLICATION_IDS:
        for item in corpus["public_surface"]["items"]:
    ...
    return {
        ...
        "opportunity_receipt_coverage": round(
            len(run["reference_complete_portfolio_receipts"]) / expected, 6
        ),
        "qualified_count": len(qualified),
        "authorized_delta_call_count": len(authorized),
        "authorized_delta_call_rate": round(
            len(authorized) / len(qualified) if qualified else 0.0, 6
        ),
        "pre_call_skip_count": len(qualified - authorized),
        "opportunity_constrained_delta_coverage": round(
            len(run["counter_compact_delta_valid_keys"])
            / len(authorized) if authorized else 0.0,
            6,
        ),
        "lineage_contract_coverage": round(
            len(run["lineage_valid_keys"]) / len(authorized)
            if authorized else 0.0,
            6,
        ),
        "pairwise_review_count": len(pairwise_calls),
        "pairwise_review_coverage": round(
            len(run["pairwise_displacement_valid_keys"])
            / pairwise_expected if pairwise_expected else 0.0,
            6,
        ),
        "fail_closed_or_reviewed_count": (
            len(authorized - set(run["lineage_valid_keys"]))
            + pairwise_expected
        ),
...
def _replacement_gate_metrics(run):
    ...
    valid_counter_count = len(
        run["pairwise_displacement_valid_keys"]
    )
    reasons = Counter(
    ...
    return {
        ...
        "provider_call_added_count": sum(
            value["stage"] == "PAIRWISE_DISPLACEMENT_REVIEW"
            for value in run["task_calls"]
        ),
    }
...
def _validate_preregistration(preregistration, corpus):
    ...
    if (
        ...
        raise ValueError("reference_complete_portfolio_preregistration_invalid")
...
def _validate_run(run, corpus, preregistration):
    ...
    if (
        ...
        raise ValueError("reference_complete_portfolio_run_invalid")
...
def _checkpoint(
    ...
    counter_valid, failures, reference_complete_portfolio_receipts,
    lineage_valid, pairwise_views, pairwise_receipts, pairwise_valid,
):
    if callback is None:
        return
    commitment = {
        "progress_version": "reference_complete_portfolio_progress_v0_51",
        "source_corpus_hash": corpus["artifact_hash"],
        "source_preregistration_hash": preregistration["artifact_hash"],
        ...
        "reference_complete_portfolio_receipts": dict(
            reference_complete_portfolio_receipts
        ),
        ...
        "lineage_valid_keys": list(lineage_valid),
        "pairwise_displacement_views": dict(pairwise_views),
        "pairwise_displacement_receipts": dict(pairwise_receipts),
        "pairwise_displacement_valid_keys": list(pairwise_valid),
