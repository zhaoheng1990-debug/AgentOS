"""Anonymous adjudication and typed reference for admission v0.84."""
...
ADJUDICATION_VERSION = "admission_v9_ternary_boundary_adjudication_v0_84"
REFERENCE_VERSION = "admission_v9_ternary_boundary_reference_v0_84"
LABEL_FIELDS = (
    "object_relation",
    "evidence_utility",
    "disposition",
...
def build_adjudication(*, packs, panel_manifest, responses):
    ...
    if (
        ...
        raise ValueError("admission_v9_annotation_lanes_invalid")
    ...
    for unit_key in sorted(public_units):
        ...
        if len({hash_payload(value) for value in semantic.values()}) == 1:
            ...
            continue
        adjudication_id = "admission-v9-adjudication-" + hash_payload([
    ...
    pack_value = {
        ...
        "staged_system_outputs": "WITHHELD",
        ...
        "instructions": (
            ...
            "use INDEPENDENT_REASSESSMENT. An explicit comparison between the "
            "target arms can independently support a null or directional "
            "effect and is not pooling merely because both arms are named "
            "together. Omission can be resolved by unambiguous coreference, "
            "but a genuinely different arm or outcome is not exact evidence. "
            "Retain context only when it materially identifies, interprets, "
            "scopes, or validates the exact target."
        ),
        "items": disagreements,
    ...
    manifest_value = {
        ...
        "total_span_count": len(agreements) + len(disagreements),
        "reference_state": (
        ...
        "baseline_outputs_exposed_to_adjudicator": False,
        "atomic_outputs_exposed_to_adjudicator": False,
        "staged_outputs_exposed_to_adjudicator": False,
...
def validate_adjudication(*, pack, manifest):
    ...
    hidden = (
        "annotator_identity_exposed_to_adjudicator",
        "benchmark_gold_exposed_to_adjudicator",
        "baseline_outputs_exposed_to_adjudicator",
        "atomic_outputs_exposed_to_adjudicator",
        "staged_outputs_exposed_to_adjudicator",
        "candidate_outputs_exposed_to_adjudicator",
    )
    if (
        ...
        or any(manifest.get(field) is not False for field in hidden)
    ):
        raise ValueError("admission_v9_adjudication_invalid")
    forbidden = ("gpt-5.6", "gemini-3.1", "openai", "google", "deepseek")
    if any(value in str(pack).casefold() for value in forbidden):
        raise ValueError("admission_v9_adjudication_identity_leak")
...
def adjudication_response_contract():
    return {
        ...
        "required_blinding_attestation": {
            ...
            "baseline_system_outputs_unavailable": True,
            "atomic_system_outputs_unavailable": True,
            "staged_system_outputs_unavailable": True,
...
def validate_adjudication_response(response, *, pack):
    ...
    if (
        ...
        raise ValueError("admission_v9_adjudication_response_binding_invalid")
    items = {
        item["adjudication_id"]: item for item in pack["items"]
    }
    decisions = response.get("decisions")
    if not isinstance(decisions, list) or len(decisions) != len(items):
        raise ValueError("admission_v9_adjudication_response_count_invalid")
    observed = []
    for decision in decisions:
        if (
            ...
            raise ValueError(
                "admission_v9_adjudication_decision_shape_invalid"
            )
        item = items.get(decision.get("adjudication_id"))
        observed.append(decision.get("adjudication_id"))
        if (
            item is None
            ...
            raise ValueError("admission_v9_adjudication_decision_invalid")
        positions = {
            value["position_id"]: _semantic_label(value)
            for value in item["anonymous_positions"]
        }
        basis = decision["decision_basis"]
        if basis in positions and _semantic_label(decision) != positions[basis]:
            raise ValueError("admission_v9_adjudication_position_mismatch")
    if len(observed) != len(set(observed)) or set(observed) != set(items):
        raise ValueError("admission_v9_adjudication_ids_invalid")
    ...
    *,
    ...
    if adjudication_pack["items"]:
        if adjudication_response is None:
            raise ValueError("admission_v9_adjudication_response_required")
    ...
    decisions = {
        ...
        for value in (adjudication_response or {"decisions": []})["decisions"]
    }
    ...
    for adjudication_id, binding in adjudication_manifest[
        ...
        labels.append({
            ...
            **_semantic_label(decisions[adjudication_id]),
