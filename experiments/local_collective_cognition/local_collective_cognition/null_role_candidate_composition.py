"""Independent truth-state and prospective null-role composition v0.40."""
...
CONTRACT_VERSION = "null_role_candidate_composition_v0_40"
...
def null_role_valued_exact_three_schema(*, item, arm_id, refs):
...
def validate_null_role_valued_receipt(*, raw_receipt, item, refs):
    failures = []
    if not isinstance(raw_receipt, dict):
        return ["NULL_ROLE_RECEIPT_NOT_OBJECT"]
    ...
    for candidate in candidates:
        ...
        if (
            ...
            failures.append(
                f"SUPPORTED_NULL_INFORMATION_ROLE_MISSING:{candidate_id}"
            )
        if (
            truth_state == "SUPPORTED_NULL"
...
def compose_null_role_valued_pool(
    *, pool, provisional_receipt, item, refs
    ...
    raw = {
        ...
        "rationale": (
            "Runtime composition from independent truth state, prospective "
            "null information role, and research value."
        ),
