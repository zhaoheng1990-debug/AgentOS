"""Runtime for selective evidence-boundary review v0.83."""

from __future__ import annotations

from agentos_kernel import ProviderTaskRouter

from .admission_v8_compiler import compile_boundary_review
from .admission_v8_contracts import validate_boundary_review
from .admission_v8_tasks import boundary_review_task
...
def run_boundary_review_panel(
    *,
    ...
    if staged_run.get("source_panel_hash") != panel.get("artifact_hash"):
        raise ValueError("admission_v8_staged_run_panel_mismatch")
    ...
    for item in panel["public_surface"]["items"]:
        ...
        contract_failures = validate_boundary_review(
            receipt=receipt,
            item=item,
            staged_partition=staged_partition,
        ...
        try:
            partitions[case_id] = compile_boundary_review(
                item=item,
                staged_partition=staged_partition,
                receipt=receipt,
            )
    ...
    value = {
        "runtime_version": "selective_boundary_review_runtime_v0_83",
        "arm_id": "A17_SELECTIVE_EVIDENCE_BOUNDARY_REVIEW",
...
def _validate_hash(value, field):
    ...
    if value.get(field) != hash_payload(commitment):
        raise ValueError(f"admission_v8_{field}_invalid")
