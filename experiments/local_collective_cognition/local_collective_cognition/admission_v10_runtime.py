"""Runtime orchestration for study-relation review v0.85."""

from __future__ import annotations

from agentos_kernel import ProviderTaskRouter

from .admission_v10_compiler import compile_study_relation_review
from .admission_v10_contracts import validate_study_relation_review
from .admission_v10_tasks import study_relation_task
...
def run_study_relation_review_panel(
    *,
    panel,
    preregistration,
    staged_run,
    ...
    if staged_run.get("source_panel_hash") != panel.get("artifact_hash"):
        raise ValueError("admission_v10_staged_run_panel_mismatch")
    ...
    for item in panel["public_surface"]["items"]:
        ...
        contract_failures = validate_study_relation_review(
            receipt=receipt,
            item=item,
            staged_partition=staged_partition,
        ...
        try:
            partitions[case_id] = compile_study_relation_review(
                item=item,
                staged_partition=staged_partition,
                receipt=receipt,
            )
    ...
    value = {
        "runtime_version": "study_relation_runtime_v0_85",
        "arm_id": "A19_STUDY_RELATION_REVIEW",
...
def _validate_hash(value, field):
    ...
    if value.get(field) != hash_payload(commitment):
        raise ValueError(f"admission_v10_{field}_invalid")
