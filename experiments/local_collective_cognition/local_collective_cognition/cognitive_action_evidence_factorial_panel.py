"""Blinded external panel for the v0.20 evidence factorial."""
...
FACTORIAL_PANEL_VERSION = "cognitive_action_evidence_factorial_external_panel_v0_20"
FACTORIAL_ADJUDICATION_VERSION = (
    "cognitive_action_evidence_factorial_external_adjudication_v0_20"
)
LANE_SPECS = (
...
def build_factorial_external_panel(*, corpus, run):
    ...
    if (
        ...
        raise ValueError("evidence_factorial_panel_run_invalid")
    panel_id = "evidence-factorial-panel-" + hash_payload([
        FACTORIAL_PANEL_VERSION,
    ...
    for lane_id, provider, model in LANE_SPECS:
        ...
        for source in corpus["public_surface"]["items"]:
            annotation_id = "evidence-factorial-annotation-" + hash_payload([
                FACTORIAL_PANEL_VERSION,
                panel_id,
                lane_id,
                source["conflict_id"],
        ...
        commitment = {
            "panel_version": FACTORIAL_PANEL_VERSION,
            "panel_id": panel_id,
            "lane_id": lane_id,
            "expected_annotator": {
                "provider": provider,
            ...
            "source_prompt_policies": "WITHHELD",
            "runtime_gate_policies": "WITHHELD",
            "prior_scores": "WITHHELD",
            "criterion_definitions": {
                "SELECTED_OBJECT": (
                    "Which candidate, if any, is semantically fixed by the request."
                ),
                "SELECTION_BASIS": (
            ...
            "instructions": (
                "Assess each object independently. Candidate option text is not "
                "itself definition evidence. A statement that a term is undefined "
                "or unspecified does not positively define it. Distinguish an "
                "ordinary underspecified surface from a missing named external "
                "rule, token, dictionary, catalog, or state model. Keep semantic "
                "selection separate from pragmatic preference. Return one coherent "
    ...
    manifest_commitment = {
        ...
        "factorial_policies_exposed_to_annotators": False,
        "lane_pack_hashes": {
        ...
        "current_phase": "AWAITING_GPT_GEMINI_FACTORIAL_TUPLES",
...
def validate_factorial_external_panel(*, packs, manifest, source_inputs=None):
    packs = tuple(packs)
    commitment = {
    ...
    if (
        ...
        or manifest.get("factorial_policies_exposed_to_annotators") is not False
        or manifest.get("reference_revision_allowed") is not False
    ):
        raise ValueError("evidence_factorial_panel_manifest_invalid")
    ids_by_lane = []
    for pack in packs:
        pack_commitment = {
        ...
        if (
            ...
            raise ValueError("evidence_factorial_panel_pack_invalid")
        forbidden = (
            "deepseek-r1",
            "legacy_source",
            "repaired_source",
            "legacy_gate",
            "repaired_gate",
            "definition_source_receipt",
        )
        if any(value in str(pack).casefold() for value in forbidden):
            raise ValueError("evidence_factorial_panel_candidate_leak")
        ids_by_lane.append(ids)
    if ids_by_lane[0] & ids_by_lane[1]:
        raise ValueError("evidence_factorial_panel_lane_alias_collision")
    if (
        source_inputs is not None
        and (packs, manifest)
        != build_factorial_external_panel(**source_inputs)
    ):
        raise ValueError("evidence_factorial_panel_semantics_invalid")
...
def validate_factorial_annotation_response(response, *, pack):
    ...
    if (
        ...
        raise ValueError("evidence_factorial_annotation_binding_invalid")
    expected_ids = {item["annotation_id"] for item in pack["items"]}
    labels = response.get("labels")
    if not isinstance(labels, list) or len(labels) != len(expected_ids):
        raise ValueError("evidence_factorial_annotation_count_invalid")
    observed = []
    for label in labels:
        if (
            ...
            raise ValueError("evidence_factorial_annotation_shape_invalid")
        ...
        if (
            ...
            raise ValueError("evidence_factorial_annotation_label_invalid")
    if len(observed) != len(set(observed)) or set(observed) != expected_ids:
        raise ValueError("evidence_factorial_annotation_ids_invalid")


def build_factorial_external_adjudication(
    *, packs, panel_manifest, responses, corpus
):
    packs, responses = tuple(packs), tuple(responses)
    validate_factorial_external_panel(packs=packs, manifest=panel_manifest)
    validate_factorial_holdout(corpus)
    ...
    if (
        ...
        raise ValueError("evidence_factorial_annotation_lanes_invalid")
    ...
    for conflict_id in sorted(public):
        ...
        if len({hash_payload(value) for value in tuples.values()}) == 1:
            ...
            continue
        adjudication_id = "evidence-factorial-adjudication-" + hash_payload([
            FACTORIAL_ADJUDICATION_VERSION,
    ...
    pack_commitment = {
        "panel_version": FACTORIAL_PANEL_VERSION,
        "adjudication_version": FACTORIAL_ADJUDICATION_VERSION,
        ...
        "instructions": (
            ...
            "Candidate option text is not definition evidence. A statement that "
            "a term is undefined does not positively define it. Distinguish "
            "positive definitions, composed constraints, ordinary soft "
            "ambiguity, and missing named external specifications. Select one "
            "anonymous full position or independently reassess the whole tuple."
        ),
        "items": items,
    ...
    manifest_commitment = {
        ...
        "factorial_policies_exposed_to_adjudicator": False,
        "reference_revision_allowed": False,
        "ground_truth_claim": False,
        "selection_authority": False,
        "retention_authority": False,
...
def validate_factorial_external_adjudication(
    *, pack, manifest, source_inputs=None
    ...
    if (
        ...
        is not False
        or manifest.get("factorial_policies_exposed_to_adjudicator")
        is not False
    ):
        raise ValueError("evidence_factorial_adjudication_invalid")
    forbidden = (
        "gpt-5.6",
        "gemini-3.1",
        "openai",
    ...
    if any(value in str(pack).casefold() for value in forbidden):
        raise ValueError("evidence_factorial_adjudication_identity_leak")
    for item in pack["items"]:
        ...
        if positions != {"POSITION_1", "POSITION_2"}:
            raise ValueError("evidence_factorial_adjudication_positions_invalid")
    if (
        source_inputs is not None
        and (pack, manifest)
        != build_factorial_external_adjudication(**source_inputs)
    ):
        raise ValueError("evidence_factorial_adjudication_semantics_invalid")
...
def validate_factorial_adjudication_response(response, *, pack):
    ...
    if (
        ...
        raise ValueError("evidence_factorial_adjudication_binding_invalid")
    item_index = {
        item["adjudication_id"]: item for item in pack["items"]
    }
    decisions = response.get("decisions")
    if not isinstance(decisions, list) or len(decisions) != len(item_index):
        raise ValueError("evidence_factorial_adjudication_count_invalid")
    observed = []
    for decision in decisions:
        if (
            ...
            raise ValueError("evidence_factorial_adjudication_shape_invalid")
        ...
        if (
            ...
            raise ValueError("evidence_factorial_adjudication_decision_invalid")
        positions = {
        ...
        if (
            ...
            raise ValueError(
                "evidence_factorial_adjudication_position_binding_invalid"
            )
    if len(observed) != len(set(observed)) or set(observed) != set(item_index):
        raise ValueError("evidence_factorial_adjudication_ids_invalid")
