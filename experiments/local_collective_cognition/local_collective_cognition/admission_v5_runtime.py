"""Runtime for atomic semantic witness admission v0.80."""

from __future__ import annotations

from agentos_kernel import ProviderTaskRouter

from .admission_v5_compiler import compile_atomic_witness
from .admission_v5_contracts import validate_atomic_witness
from .admission_v5_tasks import atomic_witness_task
...
def run_atomic_witness_panel(*, panel, preregistration, adapter):
    ...
    for item in panel["public_surface"]["items"]:
        ...
        contract_failures = validate_atomic_witness(
            receipt=receipt,
            item=item,
        ...
        try:
            partitions[item["case_id"]] = compile_atomic_witness(
                item=item,
                receipt=receipt,
            )
    ...
    value = {
        "runtime_version": "atomic_witness_runtime_v0_80",
        "arm_id": "A14_ATOMIC_SEMANTIC_WITNESS",
...
def _validate_hash(value):
    ...
    if value.get("artifact_hash") != hash_payload(commitment):
        raise ValueError("admission_v5_preregistration_hash_invalid")
