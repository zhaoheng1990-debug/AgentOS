"""Asymmetric Kernel mutation for evidence/context boundary review."""

from __future__ import annotations

from .admission_v8_facts import (
...
def compile_boundary_review(*, item, staged_partition, receipt):
    spans = {
    ...
    if set(facts) != upstream_evidence | upstream_context:
        raise ValueError("admission_v8_review_coverage_invalid")
    ...
    for span_id in sorted(facts):
        ...
        if after != before:
            mutations.append({
                ...
                "after": after,
                "basis": basis,
                "boundary_issue_codes": fact["boundary_issue_codes"],
            })
        if codes:
            conflicts.append({
                "span_id": span_id,
    ...
    if upstream_reject != set(staged_partition["rejected_span_ids"]):
        raise ValueError("admission_v8_reject_partition_mutated")
    ...
    if (
        ...
        raise ValueError("admission_v8_compiled_partition_invalid")
    value = {
        "compiler_version": "selective_boundary_review_compiler_v0_83",
