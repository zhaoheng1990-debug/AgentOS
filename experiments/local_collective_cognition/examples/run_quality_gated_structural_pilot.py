"""Run the Provider-backed contrastive-packet quality-gate pilot."""
...
from local_collective_cognition.quality_gated_structural_holdout import build_quality_gated_structural_harness  # noqa: E402
from local_collective_cognition.quality_gated_structural_runtime import QualityGatedStructuralRuntime  # noqa: E402
...
def main():
    ...
    parser.add_argument("--output", default="outputs/quality_gated_structural_pilot_v0_3.json")
    ...
    base = LocalCollectiveCognitionProtocol(
        harness=build_quality_gated_structural_harness(), small_adapters=adapters,
        baseline_adapter=None, telemetry_ledger=ledger,
        reviewer_model_id="qwen2.5-1.5b-instruct", synthesizer_model_id="gemma-2-2b-it",
    )
    experiment_id = "quality-gated-structure-" + datetime.now(timezone.utc).strftime(
        "%Y%m%dT%H%M%SZ"
    )
    failure = None
    try:
        pool.load_all()
        report = QualityGatedStructuralRuntime(base).run(experiment_id=experiment_id)
