"""Close and package v0.47 lineage-bound displacement validation."""
...
def main():
    ...
    prior_analysis = read(output / "source_v0_46_analysis.json")
    prior_closure = read(output / "source_v0_46_closure.json")
    prior_posthoc = read(output / "source_v0_46_posthoc.json")
    corpus = read(output / "lineage_displacement_corpus_frozen.json")
    preregistration = read(
        output / "lineage_displacement_preregistration.json"
    )
    run = read(output / "lineage_displacement_run.json")
    analysis = read(output / "lineage_displacement_analysis.json")
    posthoc = read(output / "posthoc_lineage_displacement.json")
    ...
    diagnostics_commitment = {
        "diagnostic_version": "lineage_displacement_diagnostics_v0_47",
        ...
        "lineage_displacement_receipt_count": len(
            run["lineage_displacement_receipts"]
        ),
    ...
    write(output / "lineage_displacement_diagnostics.json", diagnostics)
    ...
    report = "\n".join([
        ...
        (
            "- Nine qualified cells each exposed an informative-null "
            "opportunity, but every opportunity used RETAIN rather than "
            "the frozen PRIORITIZE routing disposition. Runtime therefore "
            "authorized zero delta calls and skipped all nine."
        ),
        (
            "- Seven of the nine opportunities had ADEQUATE constraint "
            "binding and all nine represented supported informative nulls "
            "with MEDIUM expected Cbit. The empty activation was caused by "
            "the final-value disposition threshold, not missing semantic "
            "opportunities."
        ),
        (
            "- Because no delta or pairwise receipt was produced, the "
            "lineage and displacement mechanisms were not empirically "
            "tested. All 24 contrasts tied at zero and cannot support a "
            "positive safety or Cbit claim."
        ),
        (
            "- The next fresh experiment should distinguish review-ready "
            "from admission-ready semantics. A supported informative null "
            "with RETAIN and adequate binding may enter lineage-bound "
            "pairwise review, while final replacement still requires the "
            "independent arbiter and hard gate. This preserves gray-zone "
            "exploration without weakening promotion."
        ),
        (
    ...
    ledger_commitment = {
        "ledger_version": "lineage_displacement_ledger_v0_47",
    ...
    replay_commitment = {
        "replay_version": "lineage_displacement_replay_v0_47",
        "working_directory": str(PACK_ROOT),
        "commands": [
            "python examples/prepare_lineage_displacement.py",
            "python examples/run_lineage_displacement.py",
            "python examples/analyze_lineage_displacement_posthoc.py",
            "python examples/finalize_lineage_displacement.py",
        ],
    ...
    rollback_commitment = {
        "rollback_version": "lineage_displacement_rollback_v0_47",
    ...
    closure_commitment = {
        "closure_version": "lineage_displacement_closure_v0_47",
        "source_ledger_hash": ledger["artifact_hash"],
        "candidate_state": analysis["candidate_state"],
        "decision": analysis["decision"],
        "lineage_displacement_gate": analysis["lineage_displacement_gate"],
        "external_semantic_panel_authorized": analysis[
            "external_semantic_panel_authorized"
        ],
        "core_integration_authorized": False,
    ...
    names = [
        "source_v0_46_analysis.json",
        "source_v0_46_closure.json",
        "source_v0_46_posthoc.json",
        "lineage_displacement_corpus_frozen.json",
        "lineage_displacement_preregistration.json",
        "lineage_displacement_progress.json",
        "lineage_displacement_run.json",
        "lineage_displacement_analysis.json",
        "posthoc_lineage_displacement.json",
        "lineage_displacement_diagnostics.json",
    ...
    inventory_commitment = {
        "inventory_version": "lineage_displacement_inventory_v0_47",
        "files": [{
            "path": name,
    ...
    pack = output / "lineage_displacement_v0_47_return_pack.zip"
    zip_pack(
        pack,
        [output / name for name in names + ["hash_inventory.json"]],
    )
    manifest_commitment = {
        "manifest_version": "lineage_displacement_manifest_v0_47",
