"""Run v0.49 warrant-complete replication."""
...
def main():
    ...
    adapter = BoundedRetryProviderAdapter(
        OpenAICompatibleJsonAdapter(OpenAICompatibleProviderSpec(
            provider_id="deepseek-warrant-complete-v0-49",
    ...
    run = run_warrant_complete_experiment(
        ...
        checkpoint_callback=lambda value: write(
            output / "warrant_complete_progress.json", value
        ),
    )
    write(output / "warrant_complete_run.json", run)
    analysis = analyze_warrant_complete_experiment(
        corpus=corpus, preregistration=preregistration, run=run
    )
    write(output / "warrant_complete_analysis.json", analysis)
    print(json.dumps({
