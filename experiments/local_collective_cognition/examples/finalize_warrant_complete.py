"""Close and package v0.49 warrant-complete replication."""
...
def main():
    output = REPO_ROOT / "outputs" / "warrant_complete_v0_49"
    prior_analysis = read(output / "source_v0_48_analysis.json")
    prior_closure = read(output / "source_v0_48_closure.json")
    prior_posthoc = read(output / "source_v0_48_posthoc.json")
    corpus = read(output / "warrant_complete_corpus_frozen.json")
    preregistration = read(
        output / "warrant_complete_preregistration.json"
    )
    run = read(output / "warrant_complete_run.json")
    analysis = read(output / "warrant_complete_analysis.json")
    posthoc = read(output / "posthoc_warrant_complete.json")
    ...
    permanent_loss_frame_count = sum(
        "losing" in value["rationale"].lower()
        or "cost of losing" in value["rationale"].lower()
        for value in pairwise_values
    )

    diagnostics_commitment = {
        "diagnostic_version": "warrant_complete_diagnostics_v0_49",
        ...
        "warrant_complete_receipt_count": len(
            run["warrant_complete_receipts"]
        ),
    ...
    write(output / "warrant_complete_diagnostics.json", diagnostics)
    ...
    report = "\n".join([
        ...
        (
            f"- {posthoc['classification_distribution'].get('BENEFICIAL_REJECTED', 0)} "
            "beneficial proposals were rejected. "
            f"{permanent_loss_frame_count}/{len(pairwise_values)} pairwise "
            "rationales framed top-three displacement as losing the base "
            "hypothesis, even though source candidates remain preserved in "
            "the pool and raw receipts. This identifies non-destructive "
            "portfolio semantics as the next bottleneck."
        ),
        (
    ...
    ledger_commitment = {
        "ledger_version": "warrant_complete_ledger_v0_49",
    ...
    replay_commitment = {
        "replay_version": "warrant_complete_replay_v0_49",
        "working_directory": str(PACK_ROOT),
        "commands": [
            "python examples/prepare_warrant_complete.py",
            "python examples/run_warrant_complete.py",
            "python examples/analyze_warrant_complete_posthoc.py",
            "python examples/finalize_warrant_complete.py",
        ],
    ...
    rollback_commitment = {
        "rollback_version": "warrant_complete_rollback_v0_49",
    ...
    closure_commitment = {
        "closure_version": "warrant_complete_closure_v0_49",
        "source_ledger_hash": ledger["artifact_hash"],
        "candidate_state": analysis["candidate_state"],
        "decision": analysis["decision"],
        "warrant_complete_gate": analysis["warrant_complete_gate"],
        "external_semantic_panel_authorized": analysis[
            "external_semantic_panel_authorized"
        ],
        "core_integration_authorized": False,
    ...
    names = [
        "source_v0_48_analysis.json",
        "source_v0_48_closure.json",
        "source_v0_48_posthoc.json",
        "warrant_complete_corpus_frozen.json",
        "warrant_complete_preregistration.json",
        "warrant_complete_progress.json",
        "warrant_complete_run.json",
        "warrant_complete_analysis.json",
        "posthoc_warrant_complete.json",
        "warrant_complete_diagnostics.json",
    ...
    inventory_commitment = {
        "inventory_version": "warrant_complete_inventory_v0_49",
        "files": [{
            "path": name,
    ...
    pack = output / "warrant_complete_v0_49_return_pack.zip"
    zip_pack(
        pack,
        [output / name for name in names + ["hash_inventory.json"]],
    )
    manifest_commitment = {
        "manifest_version": "warrant_complete_manifest_v0_49",
