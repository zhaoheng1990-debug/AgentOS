"""Close and package v0.45 paired common-receipt validation."""
...
def main():
    ...
    prior_analysis = read(output / "source_v0_44_analysis.json")
    prior_closure = read(output / "source_v0_44_closure.json")
    prior_posthoc = read(output / "source_v0_44_posthoc.json")
    corpus = read(output / "paired_receipt_corpus_frozen.json")
    preregistration = read(
        output / "paired_receipt_preregistration.json"
    )
    run = read(output / "paired_receipt_run.json")
    analysis = read(output / "paired_receipt_analysis.json")
    posthoc = read(output / "posthoc_paired_receipt.json")
    ...
    diagnostics_commitment = {
        "diagnostic_version": "paired_receipt_diagnostics_v0_45",
    ...
    write(output / "paired_receipt_diagnostics.json", diagnostics)
    ...
    report = "\n".join([
        ...
        (
            "- Physical tokens fell from 221335 to 113482, a 48.7 percent "
            "reduction. Shared standard cost and Provider variance cancel "
            "inside every paired contrast."
        ),
        (
            "- Seven cells qualified. One repeated an excluded relation and "
            "was fail-closed; six received gate decisions. The gate accepted "
            "one informative null with gross +2.0 and net +1.361 Cbit."
        ),
        (
            "- Posthoc found four beneficial and two harmful proposals. The "
            "gate prevented both harmful proposals but rejected three of the "
            "four beneficial ones; positive-oracle capture was 0.250."
        ),
        (
            "- Paired measurement is now closed. The next bottleneck is "
            "pre-call routing: construct an admission-ready relation set "
            "from cross-replication receipts, quarantine state, and null "
            "semantics; skip the Provider call when this set is empty."
        ),
        (
    ...
    ledger_commitment = {
        "ledger_version": "paired_receipt_ledger_v0_45",
    ...
    replay_commitment = {
        "replay_version": "paired_receipt_replay_v0_45",
        "working_directory": str(PACK_ROOT),
        "commands": [
            "python examples/prepare_paired_receipt.py",
            "python examples/run_paired_receipt.py",
            "python examples/analyze_paired_receipt_posthoc.py",
            "python examples/finalize_paired_receipt.py",
        ],
    ...
    rollback_commitment = {
        "rollback_version": "paired_receipt_rollback_v0_45",
    ...
    closure_commitment = {
        "closure_version": "paired_receipt_closure_v0_45",
        "source_ledger_hash": ledger["artifact_hash"],
        "candidate_state": analysis["candidate_state"],
        "decision": analysis["decision"],
        "paired_receipt_gate": analysis["paired_receipt_gate"],
        "external_semantic_panel_authorized": analysis[
            "external_semantic_panel_authorized"
        ],
        "core_integration_authorized": False,
    ...
    names = [
        "source_v0_44_analysis.json",
        "source_v0_44_closure.json",
        "source_v0_44_posthoc.json",
        "paired_receipt_corpus_frozen.json",
        "paired_receipt_preregistration.json",
        "paired_receipt_progress.json",
        "paired_receipt_run.json",
        "paired_receipt_analysis.json",
        "posthoc_paired_receipt.json",
        "paired_receipt_diagnostics.json",
    ...
    inventory_commitment = {
        "inventory_version": "paired_receipt_inventory_v0_45",
        "files": [{
            "path": name,
    ...
    pack = output / "paired_receipt_v0_45_return_pack.zip"
    zip_pack(
        pack,
        [output / name for name in names + ["hash_inventory.json"]],
    )
    manifest_commitment = {
        "manifest_version": "paired_receipt_manifest_v0_45",
