"""Run v0.46 admission-opportunity routing validation."""
...
def main():
    ...
    adapter = BoundedRetryProviderAdapter(
        OpenAICompatibleJsonAdapter(OpenAICompatibleProviderSpec(
            provider_id="deepseek-opportunity-routing-v0-46",
    ...
    run = run_opportunity_routing_experiment(
        ...
        checkpoint_callback=lambda value: write(
            output / "opportunity_routing_progress.json", value
        ),
    )
    write(output / "opportunity_routing_run.json", run)
    analysis = analyze_opportunity_routing_experiment(
        corpus=corpus, preregistration=preregistration, run=run
    )
    write(output / "opportunity_routing_analysis.json", analysis)
    print(json.dumps({
