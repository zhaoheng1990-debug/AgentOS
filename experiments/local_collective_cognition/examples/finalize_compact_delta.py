"""Close and package v0.42 compact semantic-delta composition."""
...
def main():
    ...
    prior_analysis = read(output / "source_v0_41_analysis.json")
    prior_closure = read(output / "source_v0_41_closure.json")
    prior_posthoc = read(output / "source_v0_41_posthoc.json")
    corpus = read(output / "compact_delta_corpus_frozen.json")
    preregistration = read(output / "compact_delta_preregistration.json")
    run = read(output / "compact_delta_run.json")
    analysis = read(output / "compact_delta_analysis.json")
    posthoc = read(output / "posthoc_compact_delta_calibration.json")
    ...
    diagnostics_commitment = {
        "diagnostic_version": "compact_delta_diagnostics_v0_42",
        ...
        "valid_composition_count": composition[
    ...
    write(output / "compact_delta_diagnostics.json", diagnostics)

    report = "\n".join([
        "# Compact semantic-delta composition v0.42",
        "",
        "## Observed facts",
        (
        ...
        (
            "- Mean compact-delta token penalty: "
            f"{composition['mean_delta_token_penalty_cbit']:.3f} Cbit."
        ),
        (
            "- Valid compact contracts/compositions: "
            f"{posthoc['valid_delta_composition_count']}/"
            f"{posthoc['qualified_count']}; invalid contracts: "
            f"{posthoc['invalid_delta_contract_count']}."
        ),
        (
        ...
        "## Interpretation",
        (
            "- Structured excluded object pairs removed the repeated-relation "
            "failure: all six qualified deltas passed the compact contract."
        ),
        (
            "- The smaller contract reduced mean token penalty from 0.718 to "
            "0.638 Cbit, but still missed the frozen 0.600 target."
        ),
        (
            "- Only two of six qualified cells contained positive pool "
            "opportunity. Runtime composition captured both, but made harmful "
            "replacements in two oracle-tie cells."
        ),
        (
            "- Semantic score margin cannot provide a safe admission rule: "
            "both a +2.5-Cbit win and a -1.0-Cbit loss carried a margin of "
            "about fifteen points over the displaced base candidate."
        ),
        (
            "- A fresh v0.43 candidate may gate replacement without another "
            "Provider call: admit when the displaced base candidate is "
            "mechanically quarantined, or when the delta directly targets the "
            "primary object and is SUPPORTED_EFFECT, ADEQUATE, and HIGH. This "
            "rule is posthoc-derived and has no validation authority."
        ),
        (
    ...
    ledger_commitment = {
        "ledger_version": "compact_delta_ledger_v0_42",
    ...
    replay_commitment = {
        "replay_version": "compact_delta_replay_v0_42",
        "working_directory": str(PACK_ROOT),
        "commands": [
            "python examples/prepare_compact_delta.py",
            "python examples/run_compact_delta.py",
            "python examples/analyze_compact_delta_posthoc.py",
            "python examples/finalize_compact_delta.py",
        ],
    ...
    rollback_commitment = {
        "rollback_version": "compact_delta_rollback_v0_42",
    ...
    closure_commitment = {
        "closure_version": "compact_delta_closure_v0_42",
        "source_ledger_hash": ledger["artifact_hash"],
        "candidate_state": analysis["candidate_state"],
        "decision": analysis["decision"],
        "compact_delta_gate": analysis["compact_delta_gate"],
        "external_semantic_panel_authorized": analysis[
            "external_semantic_panel_authorized"
        ],
        "core_integration_authorized": False,
    ...
    names = [
        "source_v0_41_analysis.json",
        "source_v0_41_closure.json",
        "source_v0_41_posthoc.json",
        "compact_delta_corpus_frozen.json",
        "compact_delta_preregistration.json",
        "compact_delta_progress.json",
        "compact_delta_run.json",
        "compact_delta_analysis.json",
        "posthoc_compact_delta_calibration.json",
        "compact_delta_diagnostics.json",
    ...
    inventory_commitment = {
        "inventory_version": "compact_delta_inventory_v0_42",
        "files": [{
            "path": name,
    ...
    pack = output / "compact_delta_v0_42_return_pack.zip"
    zip_pack(
        pack,
        [output / name for name in names + ["hash_inventory.json"]],
    )
    manifest_commitment = {
        "manifest_version": "compact_delta_manifest_v0_42",
