"""Run v0.42 compact semantic-delta composition."""
...
def main():
    ...
    adapter = BoundedRetryProviderAdapter(
        OpenAICompatibleJsonAdapter(OpenAICompatibleProviderSpec(
            provider_id="deepseek-compact-delta-v0-42",
    ...
    run = run_compact_delta_experiment(
        ...
        checkpoint_callback=lambda value: write(
            output / "compact_delta_progress.json", value
        ),
    )
    write(output / "compact_delta_run.json", run)
    analysis = analyze_compact_delta_experiment(
        corpus=corpus, preregistration=preregistration, run=run
    )
    write(output / "compact_delta_analysis.json", analysis)
    print(json.dumps({
