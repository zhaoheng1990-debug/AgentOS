"""Run the selective, batched contrastive-structure mini pilot."""
...
from local_collective_cognition.contrastive_structural_holdout import build_contrastive_structural_harness  # noqa: E402
from local_collective_cognition.contrastive_structural_runtime import ContrastiveStructuralRuntime  # noqa: E402
...
def main():
    ...
    parser.add_argument("--output", default="outputs/contrastive_structural_pilot_v0_2.json")
    ...
    adapters = {spec.model_id: LocalProblemFormulationAdapter(
    ...
    base = LocalCollectiveCognitionProtocol(
        harness=build_contrastive_structural_harness(), small_adapters=adapters,
        baseline_adapter=None, telemetry_ledger=ledger,
        reviewer_model_id="qwen2.5-1.5b-instruct", synthesizer_model_id="gemma-2-2b-it",
    )
    experiment_id = "contrastive-structure-" + datetime.now(timezone.utc).strftime(
        "%Y%m%dT%H%M%SZ"
    )
    failure = None
    try:
        pool.load_all()
        report = ContrastiveStructuralRuntime(base).run(experiment_id=experiment_id)
    except DisagreementCaseProviderFailure as exc:
        failure = {
            "experiment_id": experiment_id, "status": "FAILED_CLOSED",
