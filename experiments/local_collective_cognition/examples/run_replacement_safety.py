"""Run v0.43 replacement-safety validation."""
...
def main():
    ...
    adapter = BoundedRetryProviderAdapter(
        OpenAICompatibleJsonAdapter(OpenAICompatibleProviderSpec(
            provider_id="deepseek-replacement-safety-v0-43",
    ...
    run = run_replacement_safety_experiment(
        ...
        checkpoint_callback=lambda value: write(
            output / "replacement_safety_progress.json", value
        ),
    )
    write(output / "replacement_safety_run.json", run)
    analysis = analyze_replacement_safety_experiment(
        corpus=corpus, preregistration=preregistration, run=run
    )
    write(output / "replacement_safety_analysis.json", analysis)
    print(json.dumps({
