"""Finalize v0.80 typed reference after Kimi adjudication."""
...
def main():
    ...
    response = read(Path(args.kimi_response)) if args.kimi_response else None
    reference = build_typed_reference(
        panel_manifest=read(output / "panel_manifest_private.json"),
        adjudication_pack=read(
            output / "kimi_k3_atomic_admission_pack_v0_80.json"
        ),
    ...
    path = output / "typed_admission_reference_candidate_v0_80.json"
    path.write_text(
