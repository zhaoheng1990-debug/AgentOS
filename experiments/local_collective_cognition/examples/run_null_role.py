"""Run v0.40 independent null-information-role composition."""
...
def main():
    ...
    adapter = BoundedRetryProviderAdapter(
        OpenAICompatibleJsonAdapter(OpenAICompatibleProviderSpec(
            provider_id="deepseek-null-role-v0-40",
    ...
    run = run_null_role_experiment(
        ...
        checkpoint_callback=lambda value: write(
            output / "null_role_progress.json", value
        ),
    )
    write(output / "null_role_run.json", run)
    analysis = analyze_null_role_experiment(
        corpus=corpus, preregistration=preregistration, run=run
    )
    write(output / "null_role_analysis.json", analysis)
    print(json.dumps({
