"""Run v0.47 lineage-bound displacement validation."""
...
def main():
    ...
    adapter = BoundedRetryProviderAdapter(
        OpenAICompatibleJsonAdapter(OpenAICompatibleProviderSpec(
            provider_id="deepseek-lineage-displacement-v0-47",
    ...
    run = run_lineage_displacement_experiment(
        ...
        checkpoint_callback=lambda value: write(
            output / "lineage_displacement_progress.json", value
        ),
    )
    write(output / "lineage_displacement_run.json", run)
    analysis = analyze_lineage_displacement_experiment(
        corpus=corpus, preregistration=preregistration, run=run
    )
    write(output / "lineage_displacement_analysis.json", analysis)
    print(json.dumps({
