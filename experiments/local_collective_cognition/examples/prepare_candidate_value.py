"""Freeze v0.37 source-blind candidate-value composition."""
...
def main():
    source = REPO_ROOT / "outputs" / "independent_arbitration_v0_36"
    analysis = read(source / "independent_arbitration_analysis.json")
    closure = read(source / "closure.json")
    posthoc = read(source / "posthoc_candidate_value_decomposition.json")
    ...
    for name, value in (
        ("source_v0_36_analysis.json", analysis),
        ("source_v0_36_closure.json", closure),
        ("source_v0_36_posthoc.json", posthoc),
        ("candidate_value_corpus_frozen.json", corpus),
        ("candidate_value_preregistration.json", preregistration),
    ):
