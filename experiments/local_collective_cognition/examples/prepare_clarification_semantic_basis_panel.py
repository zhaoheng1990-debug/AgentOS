"""Create blinded GPT-5.6 and Gemini-3.1 semantic-basis panel packs."""
...
from local_collective_cognition.clarification_semantic_basis_holdout import validate_semantic_basis_holdout_artifact
from local_collective_cognition.clarification_semantic_basis_panel import build_semantic_basis_panel, validate_semantic_basis_panel
...
def main():
    ...
    names = {"annotation-lane-a": "gpt_5_6_semantic_basis_pack.json", "annotation-lane-b": "gemini_3_1_semantic_basis_pack.json"}
    for pack in packs:
        path = output / names[pack["lane_id"]]
        write(path, pack)
        with ZipFile(path.with_suffix(".zip"), "w", compression=ZIP_DEFLATED) as archive:
            archive.write(path, arcname=path.name)
    write(output / "private_semantic_basis_panel_manifest.json", manifest)
