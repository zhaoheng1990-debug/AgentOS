"""Close and package v0.43 replacement-safety validation."""
...
def main():
    ...
    prior_analysis = read(output / "source_v0_42_analysis.json")
    prior_closure = read(output / "source_v0_42_closure.json")
    prior_posthoc = read(output / "source_v0_42_posthoc.json")
    corpus = read(output / "replacement_safety_corpus_frozen.json")
    preregistration = read(
        output / "replacement_safety_preregistration.json"
    )
    run = read(output / "replacement_safety_run.json")
    analysis = read(output / "replacement_safety_analysis.json")
    posthoc = read(output / "posthoc_replacement_safety.json")
    ...
    diagnostics_commitment = {
        "diagnostic_version": "replacement_safety_diagnostics_v0_43",
    ...
    write(output / "replacement_safety_diagnostics.json", diagnostics)

    report = "\n".join([
        "# Replacement-safety gate v0.43",
        "",
        "## Observed facts",
        (
        ...
        "## Interpretation",
        (
            "- All six qualified deltas passed the compact contract and "
            "received a zero-call replacement decision."
        ),
        (
            "- The gate accepted three proposals. Two were beneficial, but "
            "one strong-focal receipt was harmful, so the non-degradation "
            "claim failed."
        ),
        (
            "- It also rejected two beneficial informative-null proposals. "
            "Positive-oracle capture was 0.500, showing that one scalar "
            "admission lane cannot preserve both effects and nulls."
        ),
        (
            "- The harmful Bridge relation appeared in no other independent "
            "standard receipt, while the beneficial Algae relation recurred "
            "across replications. Cross-receipt structural recurrence is a "
            "candidate zero-token warrant for effect claims."
        ),
        (
            "- A fresh v0.44 candidate should retain quarantine replacement, "
            "require cross-receipt recurrence for strong-effect replacement, "
            "and add a separate informative-null lane. This rule is "
            "posthoc-derived and has no validation authority."
        ),
        (
    ...
    ledger_commitment = {
        "ledger_version": "replacement_safety_ledger_v0_43",
    ...
    replay_commitment = {
        "replay_version": "replacement_safety_replay_v0_43",
        "working_directory": str(PACK_ROOT),
        "commands": [
            "python examples/prepare_replacement_safety.py",
            "python examples/run_replacement_safety.py",
            "python examples/analyze_replacement_safety_posthoc.py",
            "python examples/finalize_replacement_safety.py",
        ],
    ...
    rollback_commitment = {
        "rollback_version": "replacement_safety_rollback_v0_43",
    ...
    closure_commitment = {
        "closure_version": "replacement_safety_closure_v0_43",
        "source_ledger_hash": ledger["artifact_hash"],
        "candidate_state": analysis["candidate_state"],
        "decision": analysis["decision"],
        "replacement_safety_gate": analysis[
            "replacement_safety_gate"
        ],
        "external_semantic_panel_authorized": analysis[
            "external_semantic_panel_authorized"
        ],
    ...
    names = [
        "source_v0_42_analysis.json",
        "source_v0_42_closure.json",
        "source_v0_42_posthoc.json",
        "replacement_safety_corpus_frozen.json",
        "replacement_safety_preregistration.json",
        "replacement_safety_progress.json",
        "replacement_safety_run.json",
        "replacement_safety_analysis.json",
        "posthoc_replacement_safety.json",
        "replacement_safety_diagnostics.json",
    ...
    inventory_commitment = {
        "inventory_version": "replacement_safety_inventory_v0_43",
        "files": [{
            "path": name,
    ...
    pack = output / "replacement_safety_v0_43_return_pack.zip"
    zip_pack(
        pack,
        [output / name for name in names + ["hash_inventory.json"]],
    )
    manifest_commitment = {
        "manifest_version": "replacement_safety_manifest_v0_43",
