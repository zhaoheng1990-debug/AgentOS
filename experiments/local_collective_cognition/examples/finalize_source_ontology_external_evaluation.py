"""Finalize the v0.21 Kimi K3 reference and source-ontology evaluation."""
...
from local_collective_cognition.cognitive_action_source_ontology_evaluation import (  # noqa: E402
    ...
    render_source_ontology_external_evaluation,
...
def main():
    ...
    parser.add_argument(
        ...
        default=(
            ...
            r"\c4258ada-e19e-445e-9f3e-af9becfa305f"
            r"\pasted-text.txt"
        ),
    )
    parser.add_argument(
    ...
    pack = read(panel_dir / "kimi_k3_source_ontology_adjudication_pack.json")
    manifest = read(
        panel_dir / "source_ontology_adjudication_manifest.json"
    )
    validate_source_ontology_adjudication_response(response, pack=pack)

    raw_path = raw_dir / "KIMI_K3_source_ontology_pasted_response.json"
    shutil.copyfile(response_path, raw_path)
    validated_path = (
        panel_dir / "KIMI_K3_validated_source_ontology_response.json"
    )
    write(validated_path, response)
    reference = build_source_ontology_external_reference(
    ...
    reference_path = (
        panel_dir / "source_ontology_external_reference_candidate.json"
    )
    evaluation_path = (
        panel_dir / "source_ontology_external_evaluation.json"
    )
    report_path = panel_dir / "SOURCE_ONTOLOGY_EXTERNAL_EVALUATION.md"
    write(reference_path, reference)
    write(evaluation_path, evaluation)
    report_path.write_text(
        render_source_ontology_external_evaluation(evaluation),
        encoding="utf-8",
    )
    ...
    diagnostics_path = (
        panel_dir / "source_ontology_external_error_diagnostics.json"
    )
    write(diagnostics_path, diagnostics)

    closure_commitment = {
        "closure_version": (
            "source_ontology_external_evaluation_closure_v0_21"
        ),
    ...
    closure_path = (
        panel_dir / "source_ontology_external_evaluation_closure.json"
    )
    write(closure_path, closure)

    pack_path = (
        output / "source_ontology_external_evaluation_v0_21_return_pack.zip"
    )
    pack_members = (
        (
            panel_dir / "kimi_k3_source_ontology_adjudication_pack.json",
            "kimi_k3_source_ontology_adjudication_pack.json",
        ),
        (
            panel_dir / "source_ontology_adjudication_manifest.json",
            "source_ontology_adjudication_manifest.json",
        ),
    ...
    inventory_commitment = {
        "inventory_version": (
            "source_ontology_external_evaluation_hash_inventory_v0_21"
        ),
        "items": [{
    ...
    write(
        panel_dir / "source_ontology_external_evaluation_hash_inventory.json",
        inventory,
    )
    print(json.dumps({
        "reference_hash": reference["artifact_hash"],
...
def build_diagnostics(corpus, evaluation):
    ...
    commitment = {
        "diagnostic_version": (
            "source_ontology_external_error_diagnostics_v0_21"
        ),
        "evaluation_hash": evaluation["artifact_hash"],
