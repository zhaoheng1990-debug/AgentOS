"""Run v0.48 review-ready routing validation."""
...
def main():
    ...
    adapter = BoundedRetryProviderAdapter(
        OpenAICompatibleJsonAdapter(OpenAICompatibleProviderSpec(
            provider_id="deepseek-review-ready-v0-48",
    ...
    run = run_review_ready_experiment(
        ...
        checkpoint_callback=lambda value: write(
            output / "review_ready_progress.json", value
        ),
    )
    write(output / "review_ready_run.json", run)
    analysis = analyze_review_ready_experiment(
        corpus=corpus, preregistration=preregistration, run=run
    )
    write(output / "review_ready_analysis.json", analysis)
    print(json.dumps({
