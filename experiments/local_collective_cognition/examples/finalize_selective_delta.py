"""Close and package v0.41 embedded selective-delta composition."""
...
def main():
    ...
    prior_analysis = read(output / "source_v0_40_analysis.json")
    prior_closure = read(output / "source_v0_40_closure.json")
    prior_posthoc = read(output / "source_v0_40_posthoc.json")
    corpus = read(output / "selective_delta_corpus_frozen.json")
    preregistration = read(output / "selective_delta_preregistration.json")
    run = read(output / "selective_delta_run.json")
    analysis = read(output / "selective_delta_analysis.json")
    posthoc = read(output / "posthoc_selective_delta_calibration.json")
    ...
    diagnostics_commitment = {
        "diagnostic_version": "selective_delta_diagnostics_v0_41",
    ...
    write(output / "selective_delta_diagnostics.json", diagnostics)
    report = "\n".join([
        "# Selective single-delta composition v0.41",
        "",
        "## Observed facts",
        (
            f"- Selective-delta-minus-A1 net mean/median: {metrics['mean']:.3f}/"
        ...
        (
            ...
            f"({composition['trigger_rate']:.3f} of cells)."
        ),
        (
        ...
        "## Interpretation",
        (
            "- The qualifier reduced eleven raw triggers to six qualified "
            "calls. All four valid delta compositions increased gross Cbit, "
            "with mean 1.625 and no gross losses."
        ),
        (
            "- Runtime composition matched the private pool oracle on every "
            "valid cell, so selection remained calibrated."
        ),
        (
            "- Two Battery deltas repeated an explicitly excluded relation, "
            "reducing delta contract coverage to 0.667. These candidates were "
            "fail-closed and never composed."
        ),
        (
            "- The one-candidate format lowered call tokens by 29%, but its "
            "0.718-Cbit penalty missed the frozen 0.6 target. The next "
            "contract should use structured excluded object pairs and a "
            "smaller semantic delta schema."
        ),
        (
    ...
    ledger_commitment = {
        "ledger_version": "selective_delta_ledger_v0_41",
    ...
    replay_commitment = {
        "replay_version": "selective_delta_replay_v0_41",
        "working_directory": str(PACK_ROOT),
        "commands": [
            "python examples/prepare_selective_delta.py",
            "python examples/run_selective_delta.py",
            "python examples/analyze_selective_delta_posthoc.py",
            "python examples/finalize_selective_delta.py",
        ],
    ...
    rollback_commitment = {
        "rollback_version": "selective_delta_rollback_v0_41",
    ...
    closure_commitment = {
        "closure_version": "selective_delta_closure_v0_41",
        "source_ledger_hash": ledger["artifact_hash"],
        "candidate_state": analysis["candidate_state"],
        "decision": analysis["decision"],
        "selective_delta_gate": analysis["selective_delta_gate"],
        "external_semantic_panel_authorized": analysis[
            "external_semantic_panel_authorized"
        ],
        "core_integration_authorized": False,
    ...
    names = [
        "source_v0_40_analysis.json",
        "source_v0_40_closure.json",
        "source_v0_40_posthoc.json",
        "selective_delta_corpus_frozen.json",
        "selective_delta_preregistration.json",
        "selective_delta_progress.json",
        "selective_delta_run.json",
        "selective_delta_analysis.json",
        "posthoc_selective_delta_calibration.json",
        "selective_delta_diagnostics.json",
    ...
    inventory_commitment = {
        "inventory_version": "selective_delta_inventory_v0_41",
        "files": [
            {
                "path": name,
    ...
    pack = output / "selective_delta_v0_41_return_pack.zip"
    zip_pack(
        pack,
        [output / name for name in names + ["hash_inventory.json"]],
    )
    manifest_commitment = {
        "manifest_version": "selective_delta_manifest_v0_41",
