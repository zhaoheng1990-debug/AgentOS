"""Run v0.44 three-lane replacement validation."""
...
def main():
    ...
    adapter = BoundedRetryProviderAdapter(
        OpenAICompatibleJsonAdapter(OpenAICompatibleProviderSpec(
            provider_id="deepseek-tri-lane-v0-44",
    ...
    run = run_tri_lane_experiment(
        ...
        checkpoint_callback=lambda value: write(
            output / "tri_lane_progress.json", value
        ),
    )
    write(output / "tri_lane_run.json", run)
    analysis = analyze_tri_lane_experiment(
        corpus=corpus, preregistration=preregistration, run=run
    )
    write(output / "tri_lane_analysis.json", analysis)
    print(json.dumps({
