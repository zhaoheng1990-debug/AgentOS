"""Run the frozen v0.29 role-collaborative stability experiment."""
...
def main():
    ...
    adapter = BoundedRetryProviderAdapter(
        OpenAICompatibleJsonAdapter(OpenAICompatibleProviderSpec(
            provider_id="deepseek-collaborative-stability-v0-29",
    ...
    run = run_collaborative_experiment(
        ...
        checkpoint_callback=lambda value: write(
            output / "collaborative_stability_progress.json", value
        ),
    )
    ...
    write(output / "collaborative_stability_run.json", run)
    write(
        output / "collaborative_stability_analysis.json",
        analysis,
    )
    print(json.dumps({
