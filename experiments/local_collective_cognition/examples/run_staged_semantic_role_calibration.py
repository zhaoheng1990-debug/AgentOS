"""Run staged context-calibrated semantic scoring on the burned v0.15 panel."""
...
def main():
    ...
    try:
        pool.load_all()
        run = run_grammar_calibration(corpus=corpus, reference=reference, role_plan=role_plan, adapters=tuple(StagedSemanticRoleAdapter(model_id=model, pool=pool) for model in MODEL_IDS))
    finally:
        pool.unload_all()
    analysis = analyze_grammar_calibration(corpus=corpus, reference=reference, role_plan=role_plan, run=run)
    write(output / "staged_semantic_run.json", run)
    write(output / "staged_semantic_analysis.json", analysis)
    (output / "STAGED_SEMANTIC_ANALYSIS.md").write_text(render_grammar_calibration_analysis(analysis).replace("Grammar-Backed", "Staged Semantic"), encoding="utf-8")
    print(json.dumps({"run_hash": run["run_hash"], "analysis_hash": analysis["artifact_hash"], "coverage": analysis["overall_coverage"], "role_metrics": analysis["role_metrics"], "routing": analysis["selected_bijective_routing"], "accounting": analysis["accounting"], "fresh_blind_collection_allowed": analysis["fresh_blind_collection_allowed"]}, indent=2, sort_keys=True))
    return 0
