"""Run v0.38 embedded self-value composition."""
...
def main():
    ...
    adapter = BoundedRetryProviderAdapter(
        OpenAICompatibleJsonAdapter(OpenAICompatibleProviderSpec(
            provider_id="deepseek-self-value-v0-38",
    ...
    run = run_self_value_experiment(
        ...
        checkpoint_callback=lambda value: write(
            output / "self_value_progress.json", value
        ),
    )
    write(output / "self_value_run.json", run)
    analysis = analyze_self_value_experiment(
        corpus=corpus, preregistration=preregistration, run=run
    )
    write(output / "self_value_analysis.json", analysis)
    print(json.dumps({
