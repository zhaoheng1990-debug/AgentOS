"""Freeze and run context-utility witness admission v0.81."""
...
from local_collective_cognition.admission_v6_evaluation import (  # noqa: E402
    build_pre_reference_evaluation,
)
from local_collective_cognition.admission_v6_fresh_holdout import (  # noqa: E402
    build_preflight_amendment,
    build_fresh_holdout,
...
from local_collective_cognition.admission_v6_runtime import (  # noqa: E402
    run_context_utility_witness_panel,
)
...
def main():
    ...
    output = ROOT / "outputs" / "admission_v6_fresh_holdout_v0_81"
    ...
    if stage == "amend":
        amendment = build_preflight_amendment(preregistration)
        validate_preflight_amendment(
            amendment,
            preregistration=preregistration,
        )
        write(output / "preflight_amendment.json", amendment)
        print(json.dumps(amendment, indent=2))
        return
    amendment = (
        read(output / "preflight_amendment.json")
        if (output / "preflight_amendment.json").exists()
        else None
    )
    validate_preregistration(
        preregistration,
    ...
    if stage == "atomic":
        ...
        return
    if stage == "candidate":
        run = run_context_utility_witness_panel(
            panel=panel,
            preregistration=preregistration,
            preflight_amendment=amendment,
            adapter=adapter,
        )
    ...
    print(json.dumps({
        ...
        "atomic_to_candidate_change_count": (
            evaluation["atomic_to_candidate_change_count"]
        ),
