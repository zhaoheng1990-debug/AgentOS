"""Run the minimal paired test of Provider-synthesized structural priors."""
...
from local_collective_cognition.structural_prior_mini_holdout import build_structural_prior_mini_harness  # noqa: E402
...
def main() -> int:
    ...
    parser.add_argument("--output", default="outputs/structural_prior_mini_pilot_v0_1.json")
    ...
    adapters = {spec.model_id: LocalProblemFormulationAdapter(
        ...
        pool=pool, telemetry_ledger=ledger, max_new_tokens=384, max_attempts=2,
    ) for spec in specs}
    ...
    experiment_id = "structural-prior-mini-" + datetime.now(timezone.utc).strftime(
        "%Y%m%dT%H%M%SZ"
    )
    try:
        pool.load_all()
        report = StructuralPriorPilotRuntime(base).run(experiment_id=experiment_id)
    finally:
        pool.unload_all()
