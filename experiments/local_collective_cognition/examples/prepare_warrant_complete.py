"""Freeze v0.49 warrant-complete replication."""
...
def main():
    source = REPO_ROOT / "outputs" / "review_ready_v0_48"
    analysis = read(source / "review_ready_analysis.json")
    closure = read(source / "closure.json")
    posthoc = read(source / "posthoc_review_ready.json")
    ...
    for name, value in (
        ("source_v0_48_analysis.json", analysis),
        ("source_v0_48_closure.json", closure),
        ("source_v0_48_posthoc.json", posthoc),
        ("warrant_complete_corpus_frozen.json", corpus),
        ("warrant_complete_preregistration.json", preregistration),
    ):
