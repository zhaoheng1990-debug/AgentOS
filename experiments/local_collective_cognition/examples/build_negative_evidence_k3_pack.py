"""Validate negative-evidence annotations and build the blind K3 dispute pack."""
...
def main():
    ...
    validate_reference_panel(
        packs=packs, manifest=panel_manifest, semantic_artifact=corpus,
    )
    ...
    _write(output / "gpt_5_6_negative_evidence_response.json", responses[0])
    _write(output / "gemini_3_1_negative_evidence_response.json", responses[1])
    pack_path = output / "kimi_k3_negative_evidence_adjudication_pack.json"
    _write(pack_path, pack)
    with ZipFile(
        pack_path.with_suffix(".zip"), "w", compression=ZIP_DEFLATED,
    ) as archive:
