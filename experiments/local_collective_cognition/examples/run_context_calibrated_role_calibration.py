"""Run context-calibrated specialist scoring on the burned v0.15 panel."""
...
from local_collective_cognition.context_calibrated_role_adapter import ContextCalibratedResidentPool, ContextCalibratedRoleAdapter  # noqa: E402
...
def main():
    ...
    pool = ContextCalibratedResidentPool(specs)
    try:
        pool.load_all()
        run = run_grammar_calibration(
            ...
            reference=reference,
            role_plan=role_plan,
            adapters=tuple(ContextCalibratedRoleAdapter(model_id=model, pool=pool) for model in MODEL_IDS),
        )
    finally:
        pool.unload_all()
    analysis = analyze_grammar_calibration(corpus=corpus, reference=reference, role_plan=role_plan, run=run)
    write(output / "context_calibrated_run.json", run)
    write(output / "context_calibrated_analysis.json", analysis)
    (output / "CONTEXT_CALIBRATED_ANALYSIS.md").write_text(render_grammar_calibration_analysis(analysis).replace("Grammar-Backed", "Context-Calibrated"), encoding="utf-8")
    print(json.dumps({
