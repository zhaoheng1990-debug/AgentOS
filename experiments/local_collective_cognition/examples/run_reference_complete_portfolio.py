"""Run v0.51 non-destructive portfolio displacement."""
...
def main():
    ...
    adapter = BoundedRetryProviderAdapter(
        OpenAICompatibleJsonAdapter(OpenAICompatibleProviderSpec(
            provider_id="deepseek-portfolio-displacement-v0-51",
    ...
    run = run_reference_complete_portfolio_experiment(
        ...
        checkpoint_callback=lambda value: write(
            output / "reference_complete_portfolio_progress.json", value
        ),
    )
    write(output / "reference_complete_portfolio_run.json", run)
    analysis = analyze_reference_complete_portfolio_experiment(
        corpus=corpus, preregistration=preregistration, run=run
    )
    write(output / "reference_complete_portfolio_analysis.json", analysis)
    print(json.dumps({
