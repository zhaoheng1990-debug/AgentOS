"""Run v0.45 paired common-receipt validation."""
...
def main():
    ...
    adapter = BoundedRetryProviderAdapter(
        OpenAICompatibleJsonAdapter(OpenAICompatibleProviderSpec(
            provider_id="deepseek-paired-receipt-v0-45",
    ...
    run = run_paired_receipt_experiment(
        ...
        checkpoint_callback=lambda value: write(
            output / "paired_receipt_progress.json", value
        ),
    )
    write(output / "paired_receipt_run.json", run)
    analysis = analyze_paired_receipt_experiment(
        corpus=corpus, preregistration=preregistration, run=run
    )
    write(output / "paired_receipt_analysis.json", analysis)
    print(json.dumps({
