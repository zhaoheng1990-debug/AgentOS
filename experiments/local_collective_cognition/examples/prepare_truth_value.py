"""Freeze v0.39 embedded truth-value composition."""
...
def main():
    source = REPO_ROOT / "outputs" / "self_value_v0_38"
    analysis = read(source / "self_value_analysis.json")
    closure = read(source / "closure.json")
    posthoc = read(source / "posthoc_self_value_calibration.json")
    ...
    for name, value in (
        ("source_v0_38_analysis.json", analysis),
        ("source_v0_38_closure.json", closure),
        ("source_v0_38_posthoc.json", posthoc),
        ("truth_value_corpus_frozen.json", corpus),
        ("truth_value_preregistration.json", preregistration),
    ):
