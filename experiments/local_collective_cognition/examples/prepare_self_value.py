"""Freeze v0.38 embedded self-value composition."""
...
def main():
    source = REPO_ROOT / "outputs" / "candidate_value_v0_37"
    analysis = read(source / "candidate_value_analysis.json")
    closure = read(source / "closure.json")
    posthoc = read(source / "posthoc_oracle_capture.json")
    ...
    for name, value in (
        ("source_v0_37_analysis.json", analysis),
        ("source_v0_37_closure.json", closure),
        ("source_v0_37_posthoc.json", posthoc),
        ("self_value_corpus_frozen.json", corpus),
        ("self_value_preregistration.json", preregistration),
    ):
