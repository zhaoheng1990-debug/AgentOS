...
def _prior():
    analysis_value = {
        ...
        "candidate_state": "REPLACEMENT_SAFETY_REJECTED_STOP",
        "physical_total_tokens": 208072,
        "runtime_metrics": {
        ...
        "composition_metrics": {
            ...
            "mean_delta_token_penalty_cbit": 0.642417,
        },
...
def test_tri_lane_experiment_can_pass_fixture():
    corpus = build_tri_lane_holdout()
    ...
    run = run_tri_lane_experiment(
        corpus=corpus,
        preregistration=preregistration,
        adapter=CompactDeltaFixture(corpus),
    )
    analysis = analyze_tri_lane_experiment(
    ...
    assert analysis["tri_lane_gate"] == "PASS"
    assert analysis["core_integration_authorized"] is False
    posthoc = analyze_tri_lane_posthoc(
