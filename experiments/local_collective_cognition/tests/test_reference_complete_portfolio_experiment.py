...
from local_collective_cognition.source_qualified_portfolio_contract import (  # noqa: E402
...
class PortfolioFixture:
    def __init__(self, corpus):
        ...
        self.profile = ProviderCapabilityProfile(
            provider_id="fixture-portfolio-displacement",
            model_id="fixture",
    ...
    def _pairwise(self, item, refs):
        ...
        return {
            ...
            "portfolio_decision": "ACTIVATE_DELTA",
            ...
            "rationale": "Fixture delta has higher marginal Cbit.",
            "evidence_refs": list(refs),
        }
    ...
    values = (
        {
            ...
            "candidate_state": "PORTFOLIO_DISPLACEMENT_REJECTED_STOP",
            "physical_total_tokens": 153036,
            "runtime_metrics": {
                "standard_compact_delta_contract_coverage": 1.0,
                "counter_compact_delta_contract_coverage": 0.8,
            },
            "composition_metrics": {
                "triggered_composition_gross_uplift": {"mean": 0.75},
            },
...
def _reference_audit(corpus):
    commitment = {
        ...
        "required_receipt_count": 16,
        "valid_receipt_count": 16,
...
def test_reference_complete_portfolio_contract_rejects_semantic_drift():
    corpus = build_reference_complete_portfolio_holdout()
    item = corpus["public_surface"]["items"][0]
    fixture = PortfolioFixture(corpus)
...
def test_reference_complete_portfolio_fixture_closes_end_to_end():
    corpus = build_reference_complete_portfolio_holdout()
    ...
    run = run_reference_complete_portfolio_experiment(
        corpus=corpus,
        preregistration=preregistration,
        adapter=PortfolioFixture(corpus),
    )
    key, portfolio_view = next(iter(
        run["pairwise_displacement_views"].items()
    ))
    portfolio_receipt = run[
        "pairwise_displacement_receipts"
    ][key]
    assert validate_source_qualified_portfolio(
        receipt=portfolio_receipt,
        item=portfolio_view,
        refs=corpus["evidence_refs"],
    ) == []
    schema = source_qualified_portfolio_schema(
        item=portfolio_view, refs=corpus["evidence_refs"]
    )
    assert schema["properties"]["base_candidate_retained"][
        "enum"
    ] == [True]
    bad_scope = {
        **portfolio_receipt,
        "portfolio_scope_understood": False,
    }
    assert "PORTFOLIO_PORTFOLIO_SCOPE_UNDERSTOOD_MISMATCH" in (
        validate_source_qualified_portfolio(
            receipt=bad_scope,
            item=portfolio_view,
            refs=corpus["evidence_refs"],
    ...
    assert portfolio_receipt["dropped_base_pool_id"].startswith("BASE:")
    assert portfolio_receipt["delta_pool_id"].startswith("COUNTER:")
    assert "candidate_id" not in portfolio_view["base_candidate"]
    assert "candidate_id" not in portfolio_view["delta_candidate"]
    local_id = {
        **portfolio_receipt,
        "dropped_base_pool_id": "C1",
    }
    assert "PORTFOLIO_DROPPED_BASE_POOL_ID_MISMATCH" in (
        validate_source_qualified_portfolio(
            receipt=local_id,
            item=portfolio_view,
            refs=corpus["evidence_refs"],
    ...
    analysis = analyze_reference_complete_portfolio_experiment(
    ...
    assert analysis["reference_complete_portfolio_gate"] == "PASS"
    assert analysis["cost_observations"][
        "affects_cognitive_quality_gate"
    ] is False
    assert all(
        receipt["permanent_deletion_performed"] is False
        and receipt["base_candidate_retained"] is True
        and receipt["delta_candidate_retained"] is True
