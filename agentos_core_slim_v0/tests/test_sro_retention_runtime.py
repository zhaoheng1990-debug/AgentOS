...
def precommitted_witness() -> SerialSelectionWitness:
    return SerialSelectionWitness(
        witness_id="PW-0002",
        status="PRECOMMITTED",
        project_scope_ref="project://sro-retention-test",
...
def test_declared_confidence_must_equal_largest_route_probability():
    decision = GradedSROCompatibilityGate().evaluate(matcher_receipt(confidence=0.77))

    assert decision.route == "ABSTAIN"
    assert decision.reason == "matcher_confidence_probability_mismatch"
    assert "invalid:confidence" in decision.hard_gate_failures
