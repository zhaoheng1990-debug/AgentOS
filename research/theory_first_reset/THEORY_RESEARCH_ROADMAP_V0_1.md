...
```mermaid
flowchart TD
    A["T0: Freeze retrospective and rollback"] --> B["T1: Formalize collective-cognition object"]
    B --> C["T2: Derive rival models and discriminating predictions"]
    C --> D["T3: Formal and synthetic identifiability tests"]
    D --> E{"Theory variable identifiable?"}
    E -- "No" --> B
    E -- "Yes" --> F["T4: Provider adequacy calibration"]
    F --> G{"Provider supports the frozen semantic object?"}
    G -- "No" --> C
    G -- "Yes" --> H["T5: Matched-budget fresh benchmark"]
    H --> I{"Positive marginal effective Cbit?"}
    I -- "No" --> J["Preserve negative result and run ObjectUpgradingAudit"]
    J --> B
    I -- "Yes" --> K["T6: Minimal modular Runtime candidate"]
    K --> L["T7: Replication, transfer, retention review"]
```
